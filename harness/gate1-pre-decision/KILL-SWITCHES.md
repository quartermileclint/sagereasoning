...
**Per-org-agent identities (Agent-Organization + Evidence Program, from 2026-07-21):** this layer applies unchanged, per credential, to every agent identity P4 provisions under `operations/agent-org-2026-07/P5-permissions-matrix.md` — revoking Tech's or Ops's (or any later agent's) consult or write-class credential kills that agent's harness the same way it killed the s9-loop identity. **Current credential ids live in `operations/agent-org-2026-07/credential-ledger.md`, not duplicated here** — that file is the single source of truth for per-agent ids so this document never drifts from it. As of 2026-07-21 the ledger is empty (no agent has been provisioned yet); this note stands ready for P4's first mint.
