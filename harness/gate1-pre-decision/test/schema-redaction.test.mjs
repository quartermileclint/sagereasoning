...
// ---------------------------------------------------------------------------
{
  ...
  // WIRE-6 — LIVE-CHECK FOLD (2026-09-06): the first cut only required the
  // count to appear SOMEWHERE in at-action-hook.mjs, and it did — on the
  // outage paths. The SUCCESS paths logged nothing, so the very case that
  // demonstrates the redaction working (a token-dense consult that returns a
  // verdict instead of failing closed) was invisible in gate1.log. Found by
  // reading the live log, not by the battery. The pin now enumerates the log
  // lines and requires each to carry it — mentor Part 4 says "per call", and a
  // successful call is a call.
  {
    const atAction = read("../claude-code/hooks/at-action-hook.mjs");
    const LINES = [
      [framing, "FRAMED (runFraming)", /\$\{logLabel\}[\s\S]{0,900}redacted=\$\{r\.redactions\}/],
      [atAction, "CONSULT (success)", /`CONSULT session=[\s\S]{0,900}redacted=\$\{r\.redactions\}/],
      [atAction, "CONSULT-OUTAGE", /CONSULT-OUTAGE[\s\S]{0,900}redacted=\$\{r\.redactions\}/],
      [atAction, "GUARD-BLOCK", /GUARD-BLOCK[\s\S]{0,900}redacted=\$\{r\.redactions\}/],
      [atAction, "GUARD-CAUTION", /GUARD-CAUTION[\s\S]{0,900}redacted=\$\{r\.redactions\}/],
      [atAction, "GUARD-PROCEED", /GUARD-PROCEED[\s\S]{0,900}redacted=\$\{r\.redactions\}/],
      [atAction, "GUARD-OUTAGE", /GUARD-OUTAGE[\s\S]{0,900}redacted=\$\{redactions\}/],
      [subagent, "DISCERN (success)", /`DISCERN session=[\s\S]{0,900}redacted=\$\{redactedTrace\.count\}/],
      [subagent, "DISCERN-OUTAGE", /DISCERN-OUTAGE[\s\S]{0,900}redacted=\$\{redactedTrace\.count\}/],
    ];
    const missing = LINES.filter(([src, , re]) => !re.test(src)).map(([, name]) => name);
    ok("WIRE-6 EVERY log line that reports a redacted call carries the count — success paths as well as outage paths (mentor Part 4: 'the redaction is logged so a future reader knows what was replaced'). Enumerated, not a single somewhere-in-the-file match",
