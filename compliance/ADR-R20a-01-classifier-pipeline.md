...
**Status:** Accepted — founder adopted recommended defaults on 15 April 2026. Located at `/compliance/`.
**Date:** 2026-04-15
...
## 5. Founder decisions — adopted 15 April 2026

- [x] **D1-c** — Two-stage classifier (rules + LLM for borderline)
- [x] **D2-a** — Anthropic small model (Haiku)
- [x] **D3-a** — YAML file in repo (`/website/src/lib/r20a-rules.yml`)
- [x] **D4-a** — Supabase table `vulnerability_flag` with R17a Tier C RLS
- [x] **D5-a** — Supabase Studio with saved query for the reviewer interface
- [x] **D6-c** — Fail open with alerting and classifier-down marker rows
- [x] **D7-b** — Variable cost budget via two-stage, tightened after hold point
- [ ] Implementation plan to follow in a separate document once founder confirms move from `/drafts/` to `/compliance/`
