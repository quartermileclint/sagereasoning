...
All tool outputs that evaluate, score, or recommend actions must include a visible disclaimer: *"This is a philosophical framework and does not consider legal, medical, financial, or personal obligations."* Journal teachings and reflective questions are exempt unless they produce evaluative output.
...
Free API access is limited to: control filter + kathekon assessment + summary proximity level. Full diagnostic output (named passions, false judgements, deliberation framework, progress tracking) requires paid tier. Paid-tier revenue must cover at least 2x the LLM API costs incurred by that tier.
