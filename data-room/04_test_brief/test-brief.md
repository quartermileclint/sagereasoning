...
| **Combination 2 — Reasoning + Assent, no Reflect, marketed as a "practice"** | **Documentation-gated** — legitimate use; unsupported *claim* | ✅ **PASSING — Verified 2026-05-27.** The no-practice disclaimer (R19e; founder-approved; recorded under **P2 2e**, not "Priority 4") is present on all four surfaces — developer docs, `llms.txt`, `agent-card.json`, limitations page — and asserted by `scripts/whole-system-harness/run-comb2.ts` (8/8). Resolves L7 assertion (b) | Docs/discovery surfaces: `npx tsx scripts/whole-system-harness/run-comb2.ts` asserts the no-practice disclaimer string appears wherever this config is offered |

### A.3 — The disclaimer itself (Priority 4 output)

| Item | Expected | 0c verification method |
|---|---|---|
| No-practice disclaimer | Present, plain-language, accurate, across docs / `llms.txt` / `agent-card.json` / limitations page | ✅ **Done 2026-05-27.** Text authored under R19e (recorded under **P2 2e** — "Priority 4" was a stale label; P4 is Stripe). Verify: `cd website && npx tsx scripts/whole-system-harness/run-comb2.ts` asserts the string present on each surface (8/8 PASS); or grep/open each surface manually. |
