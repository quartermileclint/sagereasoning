...
| **Known risk — ✅ CLOSED 2026-05-24 (was LOAD-BEARING; the P1 finding)** | *Historical:* the bridge's signature anchoring was **wrapper-side** — `deriveReceiptId` *hashes* the signature but does **not verify** it, and the credential write path (`POST /api/accreditation/[agent_id]`) **trusted the submitted aggregates** with no server-side Ed25519 verification (the `verifyLayer2Signature` half was unbuilt). That was the false-credential door (Combination 1). **Now closed:** `verifyLayer2Signature` was built and wired into a **synchronous server-side provenance gate** on the write path, deployed **Live 2026-05-24** behind `SUBSTRATE_PROVENANCE_GATE_ENABLED='true'`. A write with no genuine substrate signature is rejected (no `provenance` → 422 `bad_provenance`; forged → 403 `no_examination`). **R18f enforced.** See M-4 (resolved) + `D-SAGE-ASSENT-PROVENANCE-GATE-BUILD-WIRED-VERIFIED-2026-05-24`. (Note: the *aggregate-faithfulness* residual remains — M-6.) |
| **Test must assert** | (a) a genuine `SignedLayer2Assessment` maps to a well-formed `EvaluatedAction` with a `receipt_id` derived from its signature; (b) **[PASSING]** a credential write carrying **no genuine substrate signature** is **rejected at the write boundary** (no `provenance` → 422 `bad_provenance`; forged → 403 `no_examination`) — the Combination-1 negative test, **verified in production 2026-05-24** (see `04_test_brief/`) |
