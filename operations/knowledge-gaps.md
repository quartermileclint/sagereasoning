...
**Founder redirect phrase — DELIBERATELY NOT SUPPLIED HERE.** KG-EX1 carries seven; this entry carries none, on purpose. Route (c) — the cache failure-mode row and its redirect phrase — was **ruled to need its own scoping session first**, because a phrase requiring the founder to have already diagnosed the lesson-transfer failure before deploying it inherits the exact blind spot it exists to sidestep. That scoping is **item 4 of the reflections arc**, combined with IW-7's trigger-legibility question. **Inventing a phrase here would pre-answer a deferred ruling.** When item 4 lands, its phrase belongs in `/adopted/standing-protocol-cache.md` §"AI failure modes to watch for at session open", and this line is replaced by a pointer to it.
