...
| H2 | `website/src/app/api/billing/cost-alerts/evaluate/route.ts:246` | `loop_billing_events` | none (heap) | none (whole table) | **now** — ~3,200 rows by 2026-08-25, append-only, no retention | D4 per-call spike (global): count, total, max over "all loops" | computed over an arbitrary 1,000-row subset; the daily Slack cost-health signal is wrong with no indication | **DONE 2026-09-03** — paged via `pagedRows()` on `id` (loop_billing_events's UUID PK), exhaustive client-side aggregation, no migration needed |
| H3 | `…/cost-alerts/evaluate/route.ts:131` | `loop_billing_events` | none | `agent_id not null` (whole table) | **now** | enumerate identities to evaluate | identities beyond the first 1,000 rows are silently excluded from cost evaluation | **DONE 2026-09-03** — paged via `pagedRows()` on `id`, distinct computed exhaustively client-side |
| H4 | `…/cost-alerts/evaluate/route.ts:173` and `website/src/lib/substrate/substrate-identity-baseline.ts:70` | `loop_billing_events` | none | `.eq('agent_id')` | **now** for `sagereasoning:s9-loop@v1` (the dominant consult identity; thousands of loops) | per-identity anomaly detector; identity cost baseline | loop count, total and max computed on 1,000 arbitrary rows of one agent | **DONE 2026-09-03** — paged via `pagedRows()` on `id`, both call sites (cost-alerts + substrate-identity-baseline.ts) |
| H5 | `website/src/app/api/abuse/evaluate/route.ts:134` | `substrate_audit_events` | none | `agent_id not null` (whole table) | **now** — ~3,200+ rows, append-only, DELETE revoked | enumerate identities for the A19 abuse detectors | identities silently dropped from abuse detection | **DONE 2026-09-03** — paged via `pagedRows()` on `event_id` (substrate_audit_events's UUID PK) |
| H6 | `…/abuse/evaluate/route.ts:187` | `substrate_audit_events` | none | `.eq('agent_id')` | **now** for s9-loop | velocity + structural detectors (windowed request counts, input-size sets) | detectors run on an arbitrary 1,000-event subset; a burst can be invisible | **DONE 2026-09-03** — paged via `pagedRows()` on `event_id`, plus a new disclosed 24-hour lookback bound (`REQUEST_VELOCITY_LOOKBACK_HOURS`) — ancient history has no bearing on a CURRENT burst |
| H7 | `website/src/app/api/admin/slo-health/route.ts:54` | `substrate_audit_events` | none | `.eq('surface','api_reason')` | **now** | the A14 SLO/health tracker (percentile latencies) | the SLO figure is a percentile over 1,000 arbitrary rows, not the population | **DONE 2026-09-03** — paged via `pagedRows()` on `event_id`; full history preserved (the `all_time` window statistic needs it, so unlike H6 this was NOT time-narrowed) |
...
| H10 (new 2026-09-03, promoted from M4 §2.2) | `…/admin/metrics/route.ts:83` | `analytics_events` (whole table, fallback) | none | **CONFIRMED NOW.** `select proname from pg_proc where proname='get_event_counts'` returned zero rows in production — the RPC does not exist, so `totalEvents.error` is truthy on EVERY call and the unbounded `analytics_events.select('event_type')` fallback (`admin/metrics/route.ts:83-88`) is the live path, unconditionally, on every request to this route. | the admin metrics dashboard's all-time event counts | all-time counts are silently truncated to an arbitrary 1,000 rows on every admin-metrics page load, right now | **DONE 2026-09-03** — paged via `pagedRows()` on `created_at` (no PK column name confirmable in-repo; see the helper's own disclosed residual). Client-side exhaustive pagination, no migration needed. |
| H11 (new 2026-09-03, promoted from M3 §2.2) | `website/src/app/api/admin/metrics/route.ts:42` | `analytics_events` (7-day window) | none | **near-term, not merely plausible.** Founder-confirmed 2026-09-03: **752 rows in the trailing 7 days** — roughly 107 rows/day. The rolling 7-day window is within days of crossing 1,000 at this rate, not "sometime". | the admin metrics dashboard's weekly event counts | the week counts will silently truncate within days if the rate holds or grows | **DONE 2026-09-03** — same `pagedRows()` fix as H10, applied to both the week and today windows. |
...
1. ~~**Fix now — CONFIRMED live truncation:** H10, H11~~ — **DONE 2026-09-03**
   (`D-ROW-CAP-SWEEP-C1-BUILT-2026-09-03`): both fixed via keyset pagination, no migration required.
2. ~~**Fix now, numbers people act on:** H2–H7~~ — **DONE 2026-09-03**, same commit. All nine C1
   sites (H2, H3, H4×2 call sites, H5, H6, H7, H10, H11) now read via a shared, tested, mutation-
   verified `pagedRows()` helper (`website/src/lib/db/paged-select.ts`) instead of a new SQL RPC/view
   — client-side exhaustive pagination gives the exact same correctness as a SQL aggregate without a
   production migration, so this closed WITHOUT a founder-walked TEST-then-production step. H6 also
   gained a disclosed, deliberate narrowing (a 24-hour lookback bound on the burst-detector's read,
   `REQUEST_VELOCITY_LOOKBACK_HOURS` — ancient history has no bearing on whether an identity is
   bursting NOW; combined with pagination as defense-in-depth). Sweep re-run: candidate count 87→75,
   zero unbounded reads remain in any of the four touched files.
3. **Data-rights class:** M5/M6 — one shared paging helper for every export/access/delete-driving
   ...
   Its own `code-critical`, PR19-REQUIRED session per the standing instruction. **Not yet built —
   the natural next session; `pagedRows()` from item 2 may be directly reusable here.**
