...
### Row 3 — Growth *(drafted this session, P5b; recommended founder-sign target)*

| Field | Value |
|---|---|
| **Rank / status** | P1 rank 3. Deferred at the original P5 session (P1 §5 and this row's own prior text: "its capability/spend/external-account needs plausibly differ from Tech/Ops's pattern… which this session did not have grounds to reason through carefully without either overfitting the Tech/Ops template or inventing needs"). Drafted in full this session (P5b — `operations/handoffs/founder/2026-07-21-P5b-growth-permissions-matrix-row-NEXT-SESSION-PROMPT.md`), grounded in Growth's actual remit (P1 §3/§4/§5, `operations/handoffs/growth/growth-wiring-fix-close.md`) rather than the Tech/Ops template. |
| **Illustrative K1 identity** | `sagereasoning:org-growth@v1` (illustrative only — P4's election E2 decides the real scheme) |
| **UPC credential pair** | Consult credential + write-class credential, same two-credential pattern as Tech/Ops (per plan §3-P4(b)) |
| **Capabilities — consult credential** | `[consult]`. **`l1_supply` deliberately WITHHELD** — same reasoning as Tech's and Ops's rows: no demonstrated need to bypass server-side extraction, and granting it by default would quietly weaken the very safety extraction Growth's own consults should be examined by. |
| **Capabilities — write credential** | `[accreditation_write, calling, reflect]`. All three travel together per the closed vocabulary's write-class grouping (they bind `agent_id` at the write boundary, 6e §A) — a `calling`-less credential would make Growth's own declared purpose read as decorative, the exact failure mode P4(a) is written to avoid. None withheld, for the same reason as Tech's and Ops's rows: Growth is expected to run real, attended Claude-Code-loop sessions from day one. |
| **Spend envelope — consult** | Recommended `monthly_limit: 120`, `daily_limit: 10`. **Reasoning, genuinely from Growth's own call shape, not copied by analogy:** grounding this row surfaced a concrete mechanical finding — the harness's H3 at-action consult (`hooks.json` matcher `Bash\|Edit\|Write\|MultiEdit\|NotebookEdit`) never fires on WebSearch/WebFetch, Growth's dominant real tool per P1 §3/§5 ("drafting blog/social content and running competitive web research"), and that matcher is fixed harness-wide, not per-agent configurable. So Growth's actual consult *volume* per session plausibly runs **lower** than Tech's or Ops's — fewer file-mutation moments per unit of real research-heavy work; Track B's evidence dividend applies to Growth, just more thinly than to Tech/Ops. That finding argues for headroom being cheap, **not** for cutting the ceiling below a level (Ops's own 120/10) already proven not to starve a real working session — inventing a lower number with no usage data to justify it would repeat, in the opposite direction, the exact mistake this matrix exists to avoid (the documented 30/1/1 starve-class). Matching Ops's order of magnitude is the defensible choice; low utilization in practice on research-heavy days is the expected, healthy outcome, not a signal the cap was set wrong. Founder-approved as drafted, 2026-07-22 (see §6). |
| **Spend envelope — write** | Recommended `monthly_limit: 120`, `daily_limit: 10`, `max_chain_iterations: 1` (no chaining needed for a single agent's own writes). |
| **External accounts** | **None named at initial provisioning.** No Slack-workspace or analytics-tool convention was found for any non-founder identity (P1 §2/§6). Wiring analytics-events or agent-discovery signals into Growth's own market-signals channel is explicitly deferred pending its own ADR (D-Growth-3, `growth-wiring-fix-close.md`) — an external analytics account is not warranted ahead of that design decision. Not granted now; available on demonstrated need, its own future founder-walked step. |
| **Proactive / unattended envelope** | **None approved.** Attended-only, per §2's default, effective immediately on provisioning. Named future candidate (informational only, not approved): flagging the stale weekly environmental scan's growth domain (last refreshed 2026-07-20 as of this drafting) or drafting a competitive-intel note when a scan surfaces something new — a read-and-flag/draft task with no side effect beyond a note for founder review. Not approved, not scheduled. |
| **Operating surface (E1)** | **Claude-Code-loop + Gate-1 harness — founder-elected this session (2026-07-22)**, over the website-runtime alternative that P1 left open ("or website runtime if the founder prefers a lighter-weight rollout for Growth specifically") and over a narrower review-only variant, despite the named consult-trigger mismatch above. Uniform rollout pattern across all three org agents (Tech, Ops, Growth) preferred over a split or narrowed posture. |
| **Rationale** | P1: "moderate" org-urgency, "moderate" evidence-dividend fit — real value, just structurally thinner than Tech/Ops's file-and-state-centric work, because the harness's consult trigger doesn't reach Growth's dominant research tool. The thinness is disclosed here, not hidden: a future P4 (Growth) session's own calling document should set expectations accordingly (few consults on research-heavy days is normal, not a defect), and whoever runs that session may separately want to consider — as its own harness-code change, **not decided here** — whether WebSearch/WebFetch should be added to the H3 consult trigger. A genuine open question this row surfaces but does not resolve. |
...
- **Growth row: SIGNED, 2026-07-22** (session P5b) — approved as drafted (capabilities, the 120/10/1 spend envelope reasoned from Growth's own WebSearch/WebFetch-dominant call shape rather than copied from Tech/Ops, external-account answer, and attended-only default all stand unchanged). E1 resolved as Claude-Code-loop + Gate-1 harness, over the website-runtime alternative P1 left open. **P4's first mint for Growth (agent 3) is unblocked by this signature.**
- Support / Mentor rows: not signed this session (§3 rows 4–5 — deferred/excluded for the reasons stated there, not merely unsigned by omission).
...
- `operations/agent-org-2026-07/credential-ledger.md` (stood up at the original P5 session, populated at P4)
- `operations/handoffs/growth/growth-wiring-fix-close.md` (Growth's own real-remit grounding, P5b session)

*End of matrix (this pass). Rows 1–3 are founder-signed, drafted from each agent's own remit; rows 4–5 are named and reasoned, not silently skipped.*
