...
| — | **Follow-up (named, non-blocking):** `cost_health_snapshots` table missing in production | 🟡 IN PROGRESS (2026-07-22) | **Schema now applied — table + index + unique constraint + RLS lockdown (RLS enabled, zero policies, zero anon/authenticated grants) verified live on BOTH TEST and production** (`website/supabase-cost-health-snapshots-migration.sql`, all seven §VERIFY checks clean both environments). **Row-seeding + the end-to-end confirmation that the founder-hub Ops persona now reads real data (not the stub) is carried forward** — blocked this session on an unrelated admin-login access issue (see the new #13 follow-up row below), not on anything schema-side. Next session: seed one row via an authenticated `GET /api/billing/usage-summary` call, confirm the row landed, confirm the Ops Channel-1 context reads it, then close. |
...
| — | **Follow-up (named, non-blocking):** Supabase's native "Send password recovery" admin action dead-ends | 🟡 OPEN (found 2026-07-22) | This is a NEW finding, not a reversal of #13's verified claim — magic-link still genuinely works and is still the intended recovery path. But the founder tried the Supabase Dashboard's own "Send password recovery" button (Auth → Users) tonight and hit a silent, unexplained dead end: the recovery link signs the browser in via a temporary session, and `auth/page.tsx`'s `checkExistingSession` effect redirects to `/dashboard` immediately — before any password-entry UI could appear (there is none) — because it doesn't distinguish Supabase's `PASSWORD_RECOVERY` auth-state-change event from an ordinary `SIGNED_IN`. Not a launch blocker (magic-link is a working substitute), but a real footgun for anyone (admin or user) who reaches for the dashboard's recovery button. Decision-session prompt spawned (task chip) to weigh: document-only / make the dead end honest / build a real reset flow — deliberately not prescribing which. |
...
**Named non-blocking follow-up, now partially closed (2026-07-22):** `cost_health_snapshots`'s schema is applied + verified live on both TEST and production (table, unique constraint, index, RLS lockdown matching its two siblings). Seeding the first real row and confirming the founder-hub Ops persona reads it are carried to a follow-up session, blocked tonight by an unrelated admin-login access issue — not a schema or safety gate.

**Named non-blocking follow-up (found 2026-07-22, while working the item above):** Supabase's own "Send password recovery" admin action (Auth → Users dashboard) dead-ends — it signs the browser in via a temporary session and `auth/page.tsx` redirects to `/dashboard` before any password-entry UI could appear, because the app never distinguishes Supabase's `PASSWORD_RECOVERY` event from an ordinary sign-in. This does **not** reverse #13's verified claim — magic-link genuinely works and is still the intended recovery path — but it is a real, lived footgun for anyone who reaches for the dashboard's native recovery button instead. Not a launch blocker; a decision-session prompt is queued to weigh options (document-only / make the dead end honest / build a real reset flow) without presuming which.
