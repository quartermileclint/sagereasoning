...
| **Session token cost** | **Filled 2026-06-11 (verdict-memo session, ~22:00 AEST) from the still-open leg-A window (founder-pasted, verdict-memo session):** input 31.0k · output 1.8k · cache read 17.8M · cache write 1.1M · **total 18.9M tokens**. Usage-credits dollar figure showed $0.00 of $100.00 (plan usage; no per-session dollar amount available). *Caveat: output-token figure low relative to documents written — possibly a display/compaction artefact; recorded verbatim as supplied (KG5). Same caveat applies to leg B's row.* |
