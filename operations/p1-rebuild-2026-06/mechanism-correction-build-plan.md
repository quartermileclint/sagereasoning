...
> **CLOSED 2026-07-11 (`D-MENTOR-VERDICTS-CALLING-REFLECTION-GATE2-ADOPTED`, G7):** after the M4 parking, the gate engine was replaced by the full deterministic Layer-2 sandwich (2026-06-19 port), and the mentor ruled the amendment discharged iff the quick tier runs the value-classification component. **Verified same-day at the code level: it does — value classification is depth-invariant on the credential-bearing surfaces** (neither `extractFeatures` nor `applyMechanisms` takes a depth parameter; `DEPTH_MECHANISMS` belongs only to the legacy prose engine, which carries no credential). No single-question patch and no screened label owed; the end-to-end pin rides the trust-layer S9 batteries; the screened-label rule re-engages if any surface ever credentials the legacy bundled-depth output. Verbatim record + verification: `operations/trust-layer-2026-07/2026-07-11-mentor-consultation-calling-reflection-gate2-verbatim.md` §3.
