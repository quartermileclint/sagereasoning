...
> — Stobaeus, *Eclogae* 2.88 (`stoic-brain/passions.json`, `key_clarification` under `definition_of_passion`; the field itself is labelled only *"Stobaeus Ecl. Section 5"*, but the file's own header maps that whole section to *"Eclogae 2.88-90"*, and the three definitions immediately preceding this passage — the same source material this clarification glosses — are each pinned to *"Stobaeus Ecl. 2.88"* specifically)
>
...
*Cases cited: R016, R019, R044, R065, R099 (all verified in `2026-08-23-stage1-extraction.md`, against the exact wording quoted). The findings record's own §1 list also named R097 as an SC-2 instance, quoting a sentence that does not appear in the extraction for that entry; it is not used here. That citation has since been corrected at source (`2026-08-23-project-reflections-findings-record.md`, §1 and §2, corrected 2026-08-25) rather than left standing — the wrongly-attributed content turned out to belong to two other entries' self-corrections, neither of which is itself a clean SC-2 instance either. Doctrine: `stoic-brain/passions.json`, `key_clarification` (the Stobaeus definition-of-passion passage and the Euripides line it records; locus 2.88, per the file's own section-to-range mapping — checked, not merely inherited from the earlier record's citation of it). PR23's own text and its two grounding instances: `adopted/project-instructions-snapshot.md`. The four misfiled decision-log entries: the placement note at the head of `operations/decision-log.md`, as in Letter I. Verification method: every quotation above was grepped against its source file directly in this session, not carried forward from the findings record's own citation of it — the same check Letter I named performing, run again here on different quotes, because the rule this letter is about does not stay checked once.*
