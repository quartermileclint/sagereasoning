...
**Date:** 2026-08-02 · **Status:** **Authored — awaiting founder approval.** Nothing here is pre-approved; every schema, flag, deploy, mint, or credential step is its own founder-walked 0c-ii. **PR19 independent adversarial review is required before the first `code-*` session's work is treated as verified** (and may be run on this plan itself before ST1 commences, at the founder's election — the Phase-3/07-29 parallel-independent-Agent precedent applies when the Workflow opt-in is absent).
