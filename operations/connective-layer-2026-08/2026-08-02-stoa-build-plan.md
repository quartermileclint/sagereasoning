...
**Status: ✅ DONE — BUILT DARK + MIGRATION APPLIED-INERT ON TEST + PROD 2026-08-03** (founder-walked §VERIFY green both — 10 CHECKs/12 constraint rows exact on TEST; table empty + inert, flag UNSET, RLS deny-all verified) (`D-STOA-ST2-ENTRY-MODEL-BUILT-DARK-2026-08-03`): migration authored (10 named CHECKs incl. the Q5b substantive-artifact standard; RLS deny-all + revoke-first exact-row grants; NO retain_until), `stoa-store` built (fail-honest, AE-1-hardened missing-table classifier), data rights wired at birth on all four surfaces **including the PR19-HIGH agent arm** (owner-NULL agent entries deleted by owned-credential resolution at account deletion; agent entries in the Art 15/20 copies), boundary 35/0 + store 65/0 with 7 live-verified mutations; PR19 ran as four parallel independent reviewers, 1 HIGH + 4 MEDIUM all folded. ST3 carried notes: declare-route rate-limit (withdraw-cycle recency lever); the #10 tag deny-class battery.
...
**Status: ✅ DONE — BUILT DARK 2026-08-03** (`D-STOA-ST3-HUMAN-SURFACE-BUILT-DARK-2026-08-03`): `/stoa` + `GET /api/stoa/entries` + `/api/mentor/stoa` CRUD, everything 503/dark behind `SUBSTRATE_STOA_ENABLED` (UNSET); **the declaration route is the twelfth R20a route-level perimeter member** (AC5 recorded decision, both halves; guard registry 102/0 at 12+2=14). PR19 ran as four parallel independent reviewers — 2 HIGH (route-layer imports unguarded; shelf-purity pin defeatable) + 4 MEDIUM (tags outside the distress composition — the headline; porous tag deny-list; record-unanchored copy pin; §J resort gaps) ALL folded, incl. tags→subject + merged-entry gating + the mild-escalation reuse + the Proxy access-set pin + the snapshot pin + a strictly-tightening comment-strip fold to the SHARED guard suite (its call checks were comment-satisfiable perimeter-wide — found by this session's own mutation testing). Boundary 83/0 · route battery 42/0 · store 65/0 · 11 live mutations bit · dark state browser-verified. ST5 checklist gains: anonymous-sign-ins-OFF check; the q-filter pagination bound. Carried: the row-level reactivation guard (mentor-flag if contested).
...
**Status: ✅ DONE — BUILT DARK 2026-08-03** (`D-STOA-ST4-AGENT-SURFACE-BUILT-DARK-2026-08-03`): the §5 item-ii election settled — the agent-declare credential rides the existing UPC `consult` capability (no schema/mint change). `GET /api/stoa/entries` extended with a credential-presence arm (dual transport, no metering, never a gate). `POST/GET/PATCH/DELETE /api/stoa/declare` (NEW) — Bearer-only, owner-bound credential required (#13), agent_id taken exclusively from the credential's own binding, never a request body. R20a/AC5 recorded decision: outside the perimeter (agent-authored text over a credential-authenticated call, per the standing exclusion) — the guard registry's exclusion comment extended by name, arrays untouched. Agent entries gained `trust_record_url`/`accreditation_url` (#19 — static links, no live probe). **R18 staged, not applied:** `operations/connective-layer-2026-08/st4-r18-docs-staged.md` (llms.txt section, `agent-card.json` extension #21 `stoa-connective-layer/v1`, api-docs bullet) — awaits founder sign-off at ST5. PR19 ran as three parallel independent reviewers — 2 MEDIUM (a verbatim/paraphrase blend in the staged doc; a disclosed test-coverage gap re: the real credential validator) both folded/disclosed; everything else independently confirmed clean (agent_id sourcing, owner-less refusal, no metering side effect, Bearer-only enforcement, no info leak, the #19 link mechanism, the R20a exclusion's soundness against Q7(b)). Boundary 86/0 · new stoa-credential battery 19/0 · store 65/0 unchanged · guard registry 102/0 unchanged.
