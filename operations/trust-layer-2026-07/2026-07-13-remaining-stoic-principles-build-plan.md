...
4. **D4 — reflect #2/#3 → confirm the flag first.** If `SUBSTRATE_REFLECT_SCREENED_EXAM_ENABLED` is **ON** (the S9b close records it set 2026-07-12), defer prod deploy — build repo-only on a branch, verify against a test agent. If **OFF**, prod deploy during the window is safe on this axis. Build-repo-only is the conservative default regardless; "let the measurement run cleanly rather than introduce variables mid-measurement."
...
**No open founder forks remain in the plan** — the mentor resolved all six. The remaining founder actions are operational (confirm the reflect flag state, D4; run the pre-deploy byte-identity check, §5; pin the frozen-set hash at window open).
...
1. **This week (the open window) — human surface + observation, no instrument change:**
   ...
   - **Let the measurement run clean** (D4/D5): reflect #2/#3 are branch-only (or test-agent) until the flag state is confirmed; do not manufacture action classes; let natural read/search/communicate actions be measured where they occur.
