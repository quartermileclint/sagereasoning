...
**S10 ADDENDUM (2026-07-12, `D-TRUST-LAYER-S10-READ-SURFACE-BUILT-DARK` — register update):** PA-6, F-1, and the R17i export-reflect-rows item are **CLOSED** (the S10 R18 sign-off memo §2–§3 records the mechanics); PA-5 and PA-10 are **disclosed on the S10 public surface** (the envelope's does-not-attest list) with structural closure carried (PA-5 → S11; PA-10 → the S2 fold wiring). **NEW carried item — `reflect-store owner-scoping` (LOW, from the S10 Opus review, finding S10-EXPORT-RIDER-CROSS-TENANT-REFLECT):** `sage_reflect_sessions` carries no owner column and agent_id is NOT owner-unique (the UPC uniqueness is the (owner_user_id, agent_id) PAIR), so BOTH the S9b `/api/user/delete` reflect leg and the S10 `/api/user/export` §2e rider scope intimate reflect content by agent_id alone — if two owners ever held the same agent_id string, one owner's export would disclose (and one owner's delete would erase) the other's reflect sessions. Zero live exposure while single-operator/pre-0h. **Closure = a schema step (owner column or unique-ownership filter) covering export AND delete together. TRIGGER: this item BLOCKS any external multi-tenant onboarding — the onboarding review must consult this register and confirm the closure before any second tenant holds a reflect-writing credential.** Also recorded: the S10 memo §9, the S11 prompt (pre-reads), and maintainer comments at both code sites.

**S11 ADDENDUM (2026-07-12, `D-TRUST-LAYER-S11-ENFORCE-GATE-MENTOR-DEFERRED` — the enforce gate examined + deferred):** the S11 flip is **DEFERRED on mentor counsel, adopted in full** (binding verdict: `2026-07-12-mentor-consultation-s11-enforce-gate-verdict-verbatim.md`, verbatim wins). Two register-relevant carries settle here: **(a) the A2 decrease-dodge is DECLARED IRREDUCIBLE** — the omission class (an agent avoids trust-*decrease* events by not narrating the harm) is the same structural self-report-omission residual the L4 answer named; no text-based fix closes it, so at the flip it becomes a **named bound on the enforcement claim** ("enforcement binds on what the examination can find, not on what was not narrated"), NOT a closure target. **(b) PA-10 stale-artifact replay stays carry-with-disclosure** for the flip (over-trust is the safer error direction under enforce; the A5 recency confidence tier at the S2-fold is the shaped closure) and is **upgraded to a named enforcement-claim bound** at the flip. **NEW gating fact for the enforce readiness gate (not a security finding — an evidence-completeness one):** the S11 assent is licensed only after **≥7 days live MEASURE + a MEASURED false-hold rate** on the at-action examination across the live distribution — and measuring that rate needs a **false-hold labelling instrument** (the current MEASURE record logs verdicts but does not label which holds were false positives). The at-action false-positive class reproduced **five times live this session** (on the session's own most role-obligated acts — one of which, the build-plan edit, correctly read `is_kathekon=true`, showing the class is intermittent, which is precisely why a *rate* is the right measure). Building/deciding that instrument is the FIRST task of the observation period; the successor prompt (`operations/handoffs/founder/2026-07-12-trust-layer-S11-observation-period-NEXT-SESSION-PROMPT.md`) carries it. The `reflect-store owner-scoping` item above is unchanged by this deferral — it gates external multi-tenant onboarding, not the single-operator enforce flip.
