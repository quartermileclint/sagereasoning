...
2. **The MANDATORY PR19 independent re-run has completed and its confirmed findings are folded.** The build session's Workflow died whole on the account session limit (6/6 finders; `wf_0d62496f-c9e`); the review was completed FIRST-HAND per PR19's codified fallback (3 folds applied — see `D-C2-C1C-ORIENTATION-READING-BUILT-DARK-REVIEW-FALLBACK-2026-08-08`), and PR19 makes an independent re-run MANDATORY before downstream reliance. Relaunch a FRESH review (not a cached resume — the folds changed the tree) after the limit reset; the persisted script is a starting point: `pr19-c2-c1c-orientation-review-wf_0d62496f-c9e.js` under the session workflows dir, or re-author from the build close's dimension list. This walk does not proceed past step 1 until that re-run's confirmed findings are folded.
