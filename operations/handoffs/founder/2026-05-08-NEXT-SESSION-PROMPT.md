...
**Scope:** bundles 7 carry-forwards related to the post-M1-CP6-cutover surface — 5 surfaced during the M1-CP6 post-audit sub-session (consumer audit on 2026-05-08) plus 1 (sub-item (vi)) added at the 2026-05-09 sub-item (i) session plus 1 (sub-item (vii)) added at the 2026-05-09 sub-item (vi) session. Sub-items (i) **[CLOSED 2026-05-09 — see `D-ITEM-I-i-AUTHFETCH-MIGRATION-2026-05-09`]** and (vi) **[CLOSED 2026-05-09 — see `D-ITEM-I-vi-AUTHFETCH-COMPANION-2026-05-09`]**. Remaining 5 sub-items: (ii) + (iii) + (iv) + (v) + (vii) — all pre-existing bugs (4) or transient browser issues (1) — not caused by the cutover. May be elected as a single bundled item or split across sessions per founder discretion. In rough priority of user impact:
...
(vii) **Payload-shape mismatch on `/api/score-conversation` + `/api/score-decision` page-side request bodies.** Surfaced during the 2026-05-09 sub-item (vi) session post-deploy verification: both call sites returned 400 (validation errors) instead of 200 once auth migration landed. The auth migration is correct; the request bodies the pages send don't match the route handlers' expected schemas. Two parts:

  - **Mentor-hub** (line 155): page sends `{ input: threadText }`; route expects `{ conversation, context, format }`. Single-field rename (`input` → `conversation`); ~5 min fix.
  - **Ops-hub** (lines 71–74): page sends `{ option1, option2 }`; route expects `{ decision: <string>, options: [<opt1>, <opt2>] }`. **Requires small design decision** — the page UI doesn't currently capture a `decision` field. Three sub-options surfaced for resolution: (a) synthesize a placeholder `decision` string from the option texts; (b) add a third text-box to the page UI for the decision question; (c) change the API contract to accept the option1/option2 shape. Each has trade-offs to weigh at session election.

  Standard tier (additive — restoring intended behaviour on already-broken-at-payload-layer code path); estimated 30–60 min total depending on ops-hub (a)/(b)/(c) choice. Mentor-hub portion ~5 min.

**Risk class:** Standard for items (i) [CLOSED], (iv), (v), (vi) [CLOSED], (vii); Elevated for items (ii) and (iii) — both touch existing user-facing functionality. Critical Change Protocol NOT engaged for any sub-item.

**Estimated total:** 1.5–3.5 hours if remaining items done in one session [items (i) and (vi) both closed 2026-05-09]. Could be split across two sessions: e.g., session 1 = items (ii) + (iii) + (vii) (Elevated hygiene + payload-shape restoration; user-facing restoration); session 2 = items (iv) + (v) (documentation + investigation).

**Pre-work founder may bring:** any preference on scope (all-in-one vs split); any preference on which sub-item is highest priority (default sequencing above is rough user-impact-priority); for sub-item (vii) ops-hub portion, any preference on the (a)/(b)/(c) design choice surfaced above.
...
- **Item I remaining bundle** (sub-items (ii)+(iii)+(iv)+(v)+(vii) — five remaining post-(vi)-closure) consolidates the audit-surfaced + 2026-05-09-surfaced carry-forwards into one focused session (~2–4 hr) or two split sessions. Practical for capturing all known pre-existing bugs in a single push.
...
**Recommendation (not prescription):** With Item I sub-items (i) and (vi) both closed 2026-05-09, the natural next step is Item I sub-items (ii) + (iii) (Elevated hygiene — column-name fixes) potentially bundled with (vii) (Standard hygiene — payload-shape fix on the two call sites the (vi) session uncovered). Then Item B + Item D as a single observation+hygiene session. Item I sub-items (iv) + (v) folded opportunistically. Item A 2F arc when founder is ready. Items E + F + G can be folded opportunistically into other sessions when their relevant surfaces are touched.
