...
**Estimated total:** ~1–1.5 hours for the remaining (iv) + (v) bundle, or split into two sub-1-hour sessions.

**Pre-work founder may bring:** any preference on scope (bundled vs split); any preference on which of (iv) / (v) is higher priority (default sequencing above is rough user-impact-priority).
...
- **Item I remaining bundle** (sub-items (iv) + (v) — two remaining post-(ii)+(iii)+(vii)-closure) — documentation update + transient browser issue investigation. ~1–1.5 hr if bundled.
...
**Recommendation (not prescription):** With Item I sub-items (i) + (ii) + (iii) + (vi) + (vii) all closed 2026-05-09, the user-impacting hygiene work is largely complete. Natural next steps in rough priority order: Item B (post-cutover watch — cheapest read-only first session, observes live behaviour against the eight open questions) + Item D (cost monitoring restoration — observability hygiene that pairs naturally with B). Then Item I sub-items (iv) + (v) bundled (documentation + diagnostic). Then Item E (Q5 promotion — pattern is now at 5+ observations and well past third-recurrence threshold). Item A 2F arc when founder is ready. Items F + G can be folded opportunistically.
