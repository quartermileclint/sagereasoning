...
**⚠ THREE THINGS R4 MUST CARRY FROM R2b:** (1) **Spec 4's flag is BLOCKED by ruling M-4** — and per the 2026-08-17 follow-up ruling the block is NOT discharged by correcting or retiring the dimension: it holds *"until the dimension is RESTORED"*, which requires a perturbation-adjusted measure. **Do not read the mean-floor fix or the retirement as satisfying the condition below.** Original framing follows: — activating the dispersion member while `computeDispositionStability` still certifies zero variance as `advanced` creates the "carrying both" state M-4 calls unsafe; the block is stated on the flag helper itself. (2) **The Q1 flag goes FIRST** — it closes an *active* mislabelling (the vetted wording went live 2026-08-16), unlike the others which enable new capability. (3) **The D4 walk BEGINS with a founder-run `SELECT`** on `justice_floor_active` for `sagereasoning:s9-loop@v1`/dikaiosyne — the harness has been writing since the 2026-07-18 clear and may have re-latched.

**◐ R2b-successors — the five mentor rulings. FIRST SUCCESSOR SESSION DONE 2026-08-17** (`D-R2B-SUCCESSOR-M4-RETURNED-M5A-CORRECTED-M5B-SCOPED-PR19-FOLDED`). **Tier: `governance` for Phases 1-2, `code-critical` for Phase 3.** No schema, no flag set, nothing activated — but Phase 3 IS code, and BOTH phases were pushed (`18e033a` documents-only, then `224e814` carrying the build; Vercel green). An earlier version of this line read "governance throughout ... nothing pushed", true only of Phase 1. Grounding ran BEFORE any election (10 agents, 5 investigate + 5 adversarially verify) and **overturned the load-bearing claim in all five dimensions**, three of them changing the founder's actual decision.
...
**PHASE 2 + 3, SAME SITTING 2026-08-17 — the M-4 ruling landed and a SIX-route safety gap was closed.**

- **M-4 RULED and ADOPTED** (`D-MENTOR-RULING-M4-RETURN-ADOPTED`; verbatim `2026-08-17-mentor-ruling-M4-return-verbatim.md`). **The return CONFIRMED the original ruling as given** — retire `disposition_stability` from agent-facing surfaces, and **let `principled → sage_like` sit structurally unreachable**: *"a ladder that accurately reflects what has not yet been demonstrated."* Re-tuning the thresholds is named as **the dishonest option** — do not. Additionally: **correct the mean-blindness now**, in the dimension's *internal logic only* (it does NOT restore the dimension to agent-facing surfaces), and **disclose both defects** alongside the retirement. **Spec 4 stays deactivated *until the dimension is restored*** — retirement alone does NOT unblock it, and a tension with the original "carrying both" reasoning is recorded, deliberately unrelitigated. *"Execute retirement on this alone. No further ruling is needed."* **M-4 EXECUTION IS CARRIED AND UNSTARTED** — the founder placed the live safety gap ahead of it.
- **⚠ R20a PERIMETER GAP CLOSED FOR SIX ROUTES, BUILT DARK** (`D-R20A-PERIMETER-GAP-CLOSURE-SIX-ROUTES-BUILT-DARK-PR19-FOLDED`). Six authenticated human-facing routes accepted practitioner free text with **no distress check at all** and were absent from `HUMAN_FACING_POST_ROUTES`, so the guard battery could not see any of them: `passion-classify`, `passion-log`, `sage-classify`, `sage-prioritise`, **plus `mentor-baseline-response` and its founder-only twin, both found by PR19**. New shared module `website/src/lib/r20a-gap-closure.ts`; check runs before each route's own validation and before any LLM call; **ONE flag (`SUBSTRATE_R20A_GAP_CLOSURE_ENABLED`) for all six, deliberately** — a half-closed safety perimeter is worse than an open one. Perimeter now **20 route-level + 2 substrate-gate**. **Nothing activated.**
- **PR19 caught two HIGH defects the build itself introduced** — mild-severity crisis resources silently dropped on the *primary* (fresh-LLM) success path of both skill routes, present only on the cached path. **Root cause worth carrying: an `Edit` with `replace_all` matched the 6-space-indented cached return while the primary return was 4-space-indented, reporting success while hitting one of two.** Both fixed. It also forced an honesty correction: **B3 is scoped to `/impulse` alone** — the extension to these routes is the builder's argued analogy, now stated as such before the argument, with membership ratification left open as a mentor question.
- **⚠ THE SWEEP COUNT MOVED 2 → 4 → 6 across three passes and SIX IS NOT CLAIMED FINAL.** The guard registry is purely additive with **no filesystem-level exhaustiveness check** — the exact failure that let these six persist. **Closing that structurally is the highest-value follow-up.** Two weaker candidates were deliberately NOT wired (`mentor-journal-week` + twin; `mentor-appendix`) — founder calls.
- **Verified:** `tsc` 0 · build exit 0, six routes registered · guard battery **186/0** (126 at open) · **mutation-verified twice** (original + PR19-found routes) · flag unset in every env file and the process env.
- **Founder decisions recorded:** R18 **Option A** signed (application carried); the gap **precedes M-5(b)**; **`ops-hub` confirmed publicly renderable** (its 2-hour-acknowledgment copy is a live public claim — correction carried); **M-2 to also correct FD-R2**.
