...
**☐ R2b-successors — the five mentor rulings, four of them carried.** M-2 (repeated-inability examination — needs the Q1 Phase-2 column, so build them together), M-3 (split part (3) into two never-pooled rates), M-4 (correct or retire `disposition_stability` — gates Spec 4), M-5 (P0: amend the R20a claim NOW + build the genuine-detection write path). Plus the AE-3 scoping step and the `stoa-boundary` #20 ruling.
