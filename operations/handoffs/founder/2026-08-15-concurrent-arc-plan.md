...
### ◐ C4 — Human-side: the RLS-vs-route-enforcement gap (+ journal) *(heading restored 2026-08-16 at C3b — the C3b block's insertion at `72dab66` had overwritten this heading line, orphaning C4's tier line and items below; restored verbatim from `1ebd95d`)* — **PARTIAL, 2026-08-16** (`D-CONCURRENT-ARC-C4-RLS-SURVEY-2026-08-16` + `-JOURNAL-DECISIONS-AND-PACE-GATE-` + `-IMPULSE-RLS-FIX-STAGED-`): **DONE** — the journal decisions (a: elapsed-hours pace gate, BUILT; b: day-55, recorded resolved), and the full Phase 1 RLS survey (22 policy files, per-table verdicts, `operations/primal-substrate-2026-08/2026-08-16-rls-route-enforcement-survey.md`; found a wider-than-expected Class C incl. the founder's own hub conversations, 2,201 rows, exposed). **STAGED, NOT APPLIED** — Phase 2 (`impulse_entries` fix): migration + proof harness authored/typechecked/committed, AC7 six-point disclosure prepared; **the PR19 review + the founder-walked TEST→prod live walk are the remaining, founder-gated steps** (unattended session could not run the two model-setting pauses, the live migration ops, or the AC7 approval). **`impulse_entries` bypass is still OPEN until that walk completes.** **CARRIED:** the founder-walked Phase 2 walk; the rest of the survey backlog (Class A/B/C beyond `impulse_entries`, order the founder elects — mentor ruled `impulse_entries` first only).
