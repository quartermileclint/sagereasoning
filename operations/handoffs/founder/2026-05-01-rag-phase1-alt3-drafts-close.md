...
**Date:** 2026-05-01 (initial close); 2026-05-01 (Option 1 + path-(b) addendum appended).
**Session scope:** Phase 1 of the alt-3 retrieval-augmented mentor design — critical-path deliverables only. **Late-session ecosystem audit and AC-18 scoping discussion** added in-session per founder direction; outcomes recorded in the addendum below.
**Status of this document:** Adopted (session close, not the deliverables it describes). Pre-addendum version archived at `/archive/2026-05-01_session-close_pre-option-1-addendum.md`.
...
## Addendum — Option 1 (AC-18 scoping correction) + Path (b) sequencing — late session

After the critical-path drafts were complete, two further pieces of work landed in this session at the founder's direction:

### A1 — Ecosystem audit (registry-level)

Founder asked for an audit of the wider SageReasoning ecosystem (Products / Tools / Agents / Engines / Reasoning / Data / Infra / Governance / Documents) to assess alt-3 impact across all 163 components in `/website/public/component-registry.json`. The agent produced a registry-level structural audit grouping affected components into six categories: (1) the current engine (`engine-sage-reason-engine`) — replaced; (2) the 15 wrapper tools — made redundant as separate logic, become Layer-3 templates; (3) the 5 hybrid tools — rebuilt as Layer-3 projections; (4) the independent scorers (sage-diagnose / sage-profile / sage-audit / sage-iterate / sage-reflect / sage-scenario) — separate but consume canonical; (5) the Trust Layer — separate pipeline, consumes canonical; (6) the journal pipeline — contributes profile prior to mechanism 5.

Audit conclusion: **one canonical rule book serves the entire ecosystem**, with three optional extensions (document quality, longitudinal pattern, journal interpretation). Single-system architecture confirmed.

Audit limitation acknowledged: registry-level only, not workflow-level. The audit established structural relationships but did not trace per-consumer end-to-end workflows.

### A2 — Evening-reflection workflow walkthrough → AC-18 scoping question

Founder asked the agent to walk through what happens when the practitioner clicks "Share with mentor" on the EveningView of `/private-mentor`. The walkthrough (26 numbered steps from button-press to mentor bubble) revealed that `/api/mentor/private/reflect` serves **two distinct flows** on the same code path:

- **Daily-reflection ritual** (morning check-in + evening reflection rituals via `submitRitual('morning' | 'evening')`).
- **Deferral-resolution surface** (alt-3's AC-15 sub-option 1b, presenting specific deferred questions from prior `OPEN_DEFERRAL` flags).

The original alt-3 architecture (D-RAG-MENTOR-ALT3-ADOPTED-2026-04-29) treated the entire `/api/mentor/private/reflect` route as the deferral-resolution surface, with AC-18 (no shareable artefact) applying to all of it. This conflated the two flows.

Founder direction: **Option 1 — AC-18 is scoped to the deferral-resolution surface only**. The daily-reflection ritual surface preserves its visible output (`katorthoma_proximity`, `sage_perspective`, `evening_prompt`, `passions_detected`, plus potentially `mentor_observation` made visible). The daily-reflection surface may be migrated to its own page; that is a UX decision in Deliverable 14a.

This is **not** a relaxation of AC-18. It is a correction to its scope. The architectural argument behind AC-18 (the philodoxia-as-reputation-mechanism reasoning) applies to the deferral-resolution surface specifically — where the deferred question is the question the engine deterministically withheld at scoring time. Producing visible output on resolution would re-introduce the reputation mechanism the architecture was designed to remove. AC-18 remains non-negotiable on that surface.

### A3 — Workflow-audit-before-Phase-1-session-2 decision (path (b))

The AC-18 scoping question raised a meta-concern: my prior ecosystem audit was registry-level, not workflow-level. The same kind of flow ambiguity that the evening-reflection walkthrough surfaced could exist in any other consumer that hasn't been workflow-audited. Three sequencing options were presented:

- **(a)** Run a workflow audit in this session → would have run long.
- **(b)** Run a dedicated audit session before Phase-1 session 2 → preserves session boundaries.
- **(c)** Hold the line on Option A as scoped, accept the risk → accepts further mid-build scope expansion.

Founder direction: **path (b)**. A dedicated audit session runs before Phase-1 session 2. The audit covers the eight R20a perimeter routes (the safety-critical scope) at the depth of the evening-reflection walkthrough. Output: Deliverable 24 — Consumer Workflow Audit (R20a Perimeter), drafted under `/drafts/rag-mentor-alt3/consumer-workflow-audit.md`. New prompt file at `/operations/handoffs/founder/2026-05-02-rag-phase1-alt3-r20a-audit-PROMPT.md`.

### A4 — File changes made in this addendum

- ✅ **D2 amended.** `/drafts/rag-mentor-alt3/canonical-framework.md` Table 4 split into Tables 4a (daily-reflection ritual surface, visible output preserved) and 4b (deferral-resolution surface, AC-18 holds). Pre-amendment version archived at `/archive/2026-05-01_canonical-framework_pre-option-1-table4-split.md`. Header status updated to note the amendment.
- ✅ **D14 split planned.** Existing Phase-1 session-2 prompt at `/operations/handoffs/founder/2026-05-01-rag-phase1-alt3-NEXT-SESSION-PROMPT.md` updated to specify D14a (daily-reflection ritual endpoint) and D14b (deferral-resolution surface). Read sequence updated to add D24 as a precondition. Pre-update version archived at `/archive/2026-05-01_phase1-session2-prompt_pre-option-1-update.md`.
- ✅ **New prompt file produced.** `/operations/handoffs/founder/2026-05-02-rag-phase1-alt3-r20a-audit-PROMPT.md` — for tomorrow's audit session.
- ✅ **This addendum.** Documents Option 1 + path (b) decisions in the session close per founder instruction.

### A5 — What this addendum does NOT change

- The three critical-path deliverables (D2, D3, D8) remain Drafted — under founder review. D2's body is amended (Table 4 split); D3 and D8 are unchanged.
- The architectural commitment AC-18 is preserved on the surface where it applies (deferral-resolution); it is NOT relaxed or revoked.
- AC-19 (reflect-endpoint-first build order) is preserved. Phase 2 pass 1 still builds the deferral-resolution surface (now D14b's specification) first.
- The decision-log entry `D-RAG-MENTOR-ALT3-PHASE1-DRAFTS-2026-05-01` remains valid. This addendum is part of the session close per founder instruction; no separate decision-log entry is added unless the founder directs otherwise. (PR7 is honoured by this addendum's documentation of Option 1 and the path-(b) sequencing decision.)
- The manifest is **not** edited in this session. The AC-18 scoping correction lives in D2 (a draft) and this session close (a handoff). Manifest amendment, if needed, follows the D6-A archive protocol and requires a separate Elevated approval — not done here.

### A6 — Risk classification (addendum-specific)

| Change | Risk | Reasoning |
|---|---|---|
| D2 Table 4 split (in-place edit on a draft) | Standard | Draft amendment with backup. No live-system effect. No code touched. Header status surfaces the amendment. |
| Phase-1 session-2 prompt update (forward-looking document) | Standard | Forward-looking handoff; backup taken; substantive scope-expansion of session 2 to add D14a/D14b split. |
| New audit-session prompt | Standard | New file under `/operations/handoffs/`; no live-system effect. |
| Session-close addendum (this section) | Standard | Documentation. |

No Critical change. No safety-critical surface touched. No PR6 perimeter engaged. No manifest edit.

### A7 — Updated Next Session Should

The "Next Session Should" section earlier in this close pointed to Phase-1 session 2 directly. Under path (b), the sequencing is:

1. **Founder review of D2, D3, D8 (the critical-path drafts).** Approval signal at the next session's open per the existing protocol.
2. **Tomorrow's session — Deliverable 24 (R20a Perimeter Workflow Audit).** Prompt at `/operations/handoffs/founder/2026-05-02-rag-phase1-alt3-r20a-audit-PROMPT.md`. Output: `/drafts/rag-mentor-alt3/consumer-workflow-audit.md`.
3. **Founder review of D24.** May surface further AC-13/AC-17/AC-18-shaped ambiguities that change Phase-1 session 2 scope.
4. **Phase-1 session 2.** Eight deliverables (D4, D9, D10, D11, D13, D14a, D14b, D15). Prompt updated at `/operations/handoffs/founder/2026-05-01-rag-phase1-alt3-NEXT-SESSION-PROMPT.md`.
5. **Phase-1 session 3.** Remaining deliverables (D1, D5, D6, D7, D12, D16–D23).

### A8 — Cross-references added by this addendum

- `/archive/2026-05-01_canonical-framework_pre-option-1-table4-split.md` (pre-amendment D2)
- `/archive/2026-05-01_phase1-session2-prompt_pre-option-1-update.md` (pre-update Phase-1 session-2 prompt)
- `/archive/2026-05-01_session-close_pre-option-1-addendum.md` (pre-addendum session close)
- `/operations/handoffs/founder/2026-05-02-rag-phase1-alt3-r20a-audit-PROMPT.md` (new — tomorrow's audit session prompt)
