...
| **S4 — NEXT** | Window-start readiness: (a) B4 follow-up from `gate1.log` **≥2026-09-08 UTC**; (b) propose F-3′'s threshold; (c) **RESOLVED 2026-09-06 BY RULING, AND IN THE OPPOSITE DIRECTION TO THE ONE ANTICIPATED.** The question was whether S5 needs an engine edit. It does — D2 is ruled to be corrected AT THE ENGINE — **but the ruling also fixes the order: the window opens FIRST to establish a baseline, and the engine correction lands AFTER.** So D2 does not gate S4; **S4 gates D2.** Leg (c) is discharged: no engine edit precedes the window, by ruling. The frozen 130-record buffer is insulated either way (capture stores the already-computed field). **Consequence the build must handle, not a reason to reorder:** landing the correction after the window opens spans the window across two tagging regimes — segment on the boundary per AE-1's S11b read-side precedent; (c′) S9 landed **✅**; (d) **the founder sets `GATE1_FALSE_HOLD_CAPTURE`** (P8a activation, window start, item D restoration, guard re-arm in one act) | read-only autonomous half + one founder act | **gated on the date (≥2026-09-08 UTC) and on (c)'s amended form**; S9 ✅, S5 ✅ | to author |
| S11-D2 | **D2 engine correction (RULED, build owed).** Correct `computeVirtueDomains` at source so dikaiosyne tags only when another party is implicated, and correct the over-broad `is_kathekon !== null` trigger **in the same pass**. **HARD SEQUENCING BY RULING: must land AFTER the false-hold window opens (S4(d)).** Its one unruled axis is **now put to the mentor and awaiting relay (F-15)** — the scope doc's **Q-D2-4**, whether the corrected tag fires on an identified circle alone or also on an asserted natural relationship with no party identified. **Do not design the predicate before F-15 returns:** a mirrored predicate would still tag dikaiosyne on a SELF-ONLY action whenever a natural-relationship factor was extracted, partially defeating the ruled correction, and the ruled kathekon-trigger fix does not close that route. Blast radius: ~12 consumers, a live score bonus, the flip's own predicate, and the published `llms.txt:548` `self_regarding` claim | code-critical, founder-walked, PR19 | **gated on S4(d)** | to author |
