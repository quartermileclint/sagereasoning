...
**Production state at session close:** **DEPLOYED + LIVE (verified).** The founder committed + pushed; Vercel deployed (no flag flip — `SUBSTRATE_GUARDRAIL_SANDWICH_ENABLED=true` unchanged); the live `/api/guardrail` smoke is GREEN (U2 → `proceed:false`, `katorthoma_proximity:"reflexive"`, `engine_attribution:"translation-sandwich"`, **no `justice_resolution`**, floor in `signed_assessment.assessment.proximity_floors.dikaiosyne:"reflexive"` with the basis surfaced in the reasoning). The gate now floors justice via the §4 native engine. Production is NOT byte-equivalent for the gate (intended standing change). `/api/reason` (§4 Live), R18f, R20a, distress, Layer-2 signing, the UPC auth path, and the standing `pre_decision_harness` dogfood marker + H1/H2 install are all untouched. **A small follow-up commit carries the record edits flipping "goes live on deploy" → "LIVE" (decision-log deploy-confirmation, CLAUDE.md, ADR/close).**
