...
5. **REQUIRED export smoke (the BATT-1 fold):** signed in at www.sagereasoning.com, run the Bearer-JWT console `probe()` (the human-routes memo pattern) against `GET /api/user/export` → the JSON contains a `sage_reflect_sessions` key; if any session rows exist for your agents, a row carries a decrypted `response_history` (or an honest error marker) and NO `response_history_ciphertext`.
