# SESSION PASTE — standing opener + slice 3 (single paste, in order)

**Founder: paste this ENTIRE file as the first message of a new session. Nothing else is needed.**

It contains three parts, in the order a session must take them:

1. **The standing session opener**, Version 2026-08-29 — reproduced **verbatim**. It is a *copy* of
   `operations/handoffs/founder/STANDING-SESSION-OPENER-grounded-foundations.md`, made 2026-08-30. If
   that standing file has been updated since, **the standing file wins** — re-sync before relying on
   this copy.
2. **A delta block for 2026-08-30**, which the opener predates. The opener states the record's last
   activity as 2026-08-26 and `git log origin/main..HEAD` as empty; **both are stale.**
3. **The task** — provenance-ledger slice 3.

---

# ══════════ PART 1 of 3 — STANDING SESSION OPENER (verbatim copy, Version 2026-08-29) ══════════
...
# ══════════ PART 2 of 3 — DELTA: what changed on 2026-08-30 ══════════

**The opener above was written 2026-08-29 and does not carry this.** Where the two conflict on any
fact below, **this block wins**; on everything else, the opener stands.

**Re-derive these yourself at open — do not trust either document on them:**

```
git log origin/main..HEAD --oneline      # the opener says empty; it was NOT at 08-30 close
git status --short
```

## What five 2026-08-30 sessions established

**1. A finding: the provenance ledger's classification side cannot run.** The write side is healthy
(**187 rows** since activation). But `classifyProvenanceArtifact` is reached only from the
accreditation-write route, which returns **409 for a `seed` write against an existing row BEFORE the
emission call** — and the harness close hook sends only `kind: "seed"`, against a row that exists.
**15 of 15 closes since activation logged `already-exists`; zero `accred=written` across the log's
whole span.** Record: `…/2026-08-30-provenance-ledger-C2-observation-input-unreachable-FINDING.md`.

**2. The slice-2 close's carried step 5 is CORRECTED** (`8a34e18`) — it instructed the founder to
*"watch the classify log lines accumulate"*, an observation that cannot succeed. Mentor-ruled to be
corrected *"before any other action"*, and it was.

**3. C2 is DISCHARGED — on SCOPE's pre-ledger exclusion, NOT the C1 empty-population precedent.**
The route there matters: this project told the mentor the population was empty; **that was false**
(`sagereasoning:stoa-q5c-smoke@v1` wrote 2026-08-12, inside the window). The mentor **accepted the
erratum, WITHDREW the original discharge**, and issued a revised conditional ruling. The condition was
verified under four independent lines and holds — the write predates activation by 14 days, and on
that date the ledger's write path **did not exist in the codebase**. See
`…/2026-08-30-mentor-ruling-provenance-ledger-C2-reachability-verbatim.md` **§ Revised ruling** and its
**§VERIFY** block.

**4. A standing methodological fact — carry this, it outlasts the arc.** **Neither obvious observable
for "did an agent write an accreditation" is sound alone:**
...
- The sound seed signal is **`created_at`**.

**5. A new read-only script:** `website/scripts/provenance-c2-discharge-tally.ts` — the ruled scoped
reading-B tally. **Run once, point-in-time, and it must NEVER be scheduled** (that would reinstate the
sync-drift shape round 6 rejected). It **imports** `classifyProvenanceArtifact` and
`resolveLongitudinalIdentity` rather than mirroring them. Baseline artifact:
`operations/agent-circles-2026-08/runs/2026-08-30/c2-discharge-baseline.json`.

## The switch-on scoreboard — the spine of this arc

| | Condition | Status |
|---|---|---|
| **C1** | Every agent coheres on identity | ✅ satisfied (empty population; re-check at switch-on) |
...
| **C4** | The `provenance_gaps` surface live | ❌ **this session's task** |

**C3 is the binding clock. Slice 5 cannot happen before late November regardless of what is done
now** — so there is no schedule pressure on slice 3.

## Two small open items — neither blocks anything

1. **A per-identity exclusion loophole**, named for the mentor: under SCOPE's literal per-identity
   reading, an identity the ledger never recorded for is excluded outright, which **at switch-on could
   exclude an agent that simply never consulted.** The C2 conclusion does not depend on it.
2. **The Q5c teardown gap** — it covered `agent_trust_events`, `agent_trust_state` and `stoa_entries`
   but **missed `agent_accreditation`**; that leftover row turned out to be load-bearing on a readiness
   threshold.

## Carried tail, unchanged

`operations/handoffs/founder/2026-08-31-slice3-or-C2-ruling-and-carried-NEXT-SESSION-PROMPT.md` remains
current **for the carried tail only** (the reflect `user_id` security fix, the RLS backlog, the
p5-force redesign, the register items). Its slice-3 half is superseded by Part 3 below.


---

# ══════════ PART 3 of 3 — THE TASK: provenance-ledger slice 3 ══════════

**Source of record:** `operations/handoffs/founder/2026-08-31-provenance-ledger-slice3-served-field-and-attestation-NEXT-SESSION-PROMPT.md` (this is a verbatim copy of its body).
