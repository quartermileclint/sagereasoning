...
## Blocked On
**POST-CLOSE UPDATE (2026-07-08, founder-walked):** steps 1 + 2 below are **DONE** — the migration is **applied + verified on BOTH TEST and PRODUCTION** (prod-inert; the consolidated verify came back green on both — 13/15 columns, 11 indexes, both owner FKs, `trg_ate_forbid_update`, RLS true/true; the immutability trigger behaviorally confirmed on TEST: an UPDATE returned `P0001 … agent_trust_events is append-only; UPDATE is forbidden`). `SUBSTRATE_TRUST_CORE_ENABLED` stays unset ⇒ production byte-equivalent. Remaining carried: steps 3 (optional flag-on TEST walk) + 4 (the future prod emission activation).

**Founder-walked steps (carried; the AI performs none of these):**
1. ✅ **DONE (2026-07-08)** — **Apply the migration** `website/supabase-agent-trust-core-migration.sql` to **TEST**, then run the VERIFY block (paste back: both tables, columns, indexes, the FK, the trigger, RLS). A TEST admin `profiles` row must exist (memory `test-admin-needs-profiles-row`).
2. ✅ **DONE (2026-07-08)** — Per **E2**, apply the same migration to **PRODUCTION INERT** (flag unset ⇒ no writes; the M6 precedent) — its own founder-walked 0c-ii.
