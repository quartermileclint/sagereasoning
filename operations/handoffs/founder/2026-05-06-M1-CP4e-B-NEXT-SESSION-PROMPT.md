...
M1-CP4e-A landed the modules + ADR amendments + orchestrator + route + harness for AC-13 Tier 1 force-clarification on `/api/reason`. The harness is at 207/207 PASS. The code is committed and (after the founder's push) deployed to Vercel — but it is INERT: the env var `TRANSLATION_SANDWICH_TIER1_SECRET` is not yet provisioned, the F7/F8/F9 fixtures are documented in ADR-005 §8.1 but not in the harness FIXTURES array, and the F1+F2 layer3 prose caches were deleted at M1-CP4e-A (currently using the deterministic fallback_prose path; LLM caches will regenerate at this session). M1-CP4e-B closes the deployment loop: it provisions the env var, adds the fixtures, runs the harness end-to-end with real Sonnet (regenerating F1+F2 layer3 caches alongside the new F7/F8/F9 extractions), implements Phase 12 fully, and completes the Critical Change Protocol's named-risk approval before declaring the AC-13 Tier 1 deployment complete.

**Calibration validation expected at this session:** the M1-CP4e-A system prompt tightening (category 12 negative examples + heuristic) successfully redirected F3 from over-firing fusion to correctly extracting `fused: false`. M1-CP4e-B's real-Sonnet F7/F8/F9 run is the first opportunity to confirm the calibration on positive (F7 must extract `fused: true`) and Layer 2 (F8 SCOPE_AMBIGUITY at Position 6; F9 TEMPORAL_AMBIGUITY at Position 2) cases.
...
2. Standalone harness reproduces 207/207 against the M1-CP4e-A baseline. Re-run command: `cd website && LAYER1_REPLAY_CACHE=1 npx tsx scripts/verify-translation-sandwich.ts`.
...
5. Anthropic API key in `.env.local`. Marginal LLM cost ~$0.50 (F7+F8+F9 Layer 1 extractions + F1+F2 layer3 prose regeneration + Phase 12 second-turn extractions).

## Part A — Open under the protocol

Read in order:
1. `/adopted/standing-protocol-cache.md` — confirms tier `code-critical`; full session-opening protocol applies; Critical Change Protocol applies before deployment.
2. `/operations/handoffs/founder/2026-05-06-sub-session-M1-CP4e-A-close.md` — predecessor close. Pay attention to the "Recalibration journey" section — it explains the harness state at session close and the open questions M1-CP4e-B should validate.
...
5. `/adopted/adr/2026-05-04-layer1-schema-specification.md` §3.12 + §4 category 12 (the tightened prompt) + §8.1 F7 fixture spec — re-skim.
...
- Status vocabulary: ADR-008 implementation moves from **Wired (no deploy)** to **Verified (deployed; observation-only logging during parallel-run)**. tier1-token.ts moves from **Wired + Verified standalone (Phase 11)** to **Verified end-to-end** after Phase 12 real-Sonnet runs.
- Model selection: **Sonnet** for Layer 1 re-extraction (per AC1 + KG2; cache Element 6 row "Layer 1 translation (alt-3)").
- Engaged rules: R0, R5, R7, R8a, R8c, R10, AC1, AC4, AC5, AC6, AC8, KG1, KG2, KG6, PR1, PR3, PR4, PR6. AC7 NOT engaged. PR5 watch-status — **promotion candidate THIS SESSION** (third recurrence if Sonnet drift surfaces on F7/F8/F9 baseline assertions; per PR5 watch-status note, the resolution is harness assertion strategy refactor — likely M1-CP4f scope, not blocking M1-CP4e-B).
...
The route at `/api/reason` will gain the ability to ask the practitioner a clarifying question when the engine cannot proceed without more information. The new env var `TRANSLATION_SANDWICH_TIER1_SECRET` is provisioned in Vercel so the engine can issue cryptographically-signed continuation tokens. The harness adds three new test fixtures (F7/F8/F9) and runs them end-to-end with real Sonnet to verify the Tier 1 detection logic fires correctly. Existing fixtures F1+F2 also get their layer3 prose caches regenerated (deleted at M1-CP4e-A; this session restores them). From the practitioner's perspective on sagereasoning.com: NO IMMEDIATE CHANGE. The user-facing path remains bundled-depth during parallel-run; the Tier 1 mechanic surfaces to users at M1-CP6 cutover (a future session).

### 2. What could break — specific failure modes

(a) **Existing single-turn requests could fail to evaluate.** The M1-CP4e-A system prompt tightening successfully fixed F3 over-firing. M1-CP4e-B confirms via real-Sonnet on F7 (positive case — must fire) and on F1–F4 baseline (must NOT fire). Mitigation: Phase 1 + Phase 4 harness assertions on F1–F4 must show `fused: false` AND no SCOPE_AMBIGUITY / TEMPORAL_AMBIGUITY fires; if any unexpectedly fire, tighten the system prompt further per PR5 worked-example discipline before deploying.

(b) **R20a distress check could be bypassed on the second turn.** The most serious failure mode. The route's amended flow runs distress check at line 173 BEFORE token validation at line ~195. AC4 invocation test: grep + execution path proof that `enforceDistressCheck` runs unconditionally on every request, before `validateContinuationToken`. Mitigation: Phase 7 of the harness asserts the import-position invariant (PASS at 207/207 baseline); pre-deployment grep of the route file confirms distress check runs first.
...
(e) **Build break on Vercel.** A `tsc` error or runtime error in the new code could break the build. Mitigation: `cd website && npx tsc --noEmit -p .` returns clean (verified at M1-CP4e-A; re-verify at M1-CP4e-B before deploy); harness 220+ checks pass before commit.

(f) **Regression on existing M1-CP4c functionality.** AC-14 Tier 3 + Tier 2 currently work. M1-CP4e-A harness is at 207/207 (recalibrated to reflect Sonnet's current behaviour on F1+F2+F5). Any regression in `applyMechanisms` could break Tier 2 + Tier 3. Mitigation: pre-existing F1–F6 fixtures continue to pass; M1-CP4e-B's harness produces all 207 existing checks PLUS the new Tier 1 phases on F7/F8/F9 PLUS Phase 12 PLUS regenerated F1+F2 LLM-prose checks (which weren't running at M1-CP4e-A close).
...
Expected: `SUMMARY: <N> / <N> checks passed` where `N >= 220` (includes F7/F8/F9 fixtures + Phase 11 + full Phase 12 + regenerated F1+F2 LLM-prose checks). All phases pass.
...
Run the harness once to confirm 207/207 baseline still holds:
```
cd website && LAYER1_REPLAY_CACHE=1 npx tsx scripts/verify-translation-sandwich.ts 2>&1 | tail -3
```

Expected: `SUMMARY: 207 / 207 checks passed` + `ALL CHECKS PASSED`.
...
Edit `website/scripts/verify-translation-sandwich.ts`. Find the FIXTURES array (line ~269 at M1-CP4e-A close). Append three entries per the specs in ADR-005 §8.1 (F7 element-fusion case, F8 scope-ambiguity case, F9 temporal-ambiguity case).

For each fixture, set the `expected_non_empty` and `expected_optional` arrays carefully — F7 should expect `element_fusion_detected.fused === true` (handled via Phase 4 assertion); F8 + F9 expect `fused === false` with structural Layer 1 conditions per ADR-005 §8.2 Phase 1 assertions.

### Step 4 — Run the harness without LAYER1_REPLAY_CACHE to extract + cache F7/F8/F9 + regenerate F1+F2 layer3

```
cd "/Users/clintonaitkenhead/Claude-work/PROJECTS/sagereasoning/website" && npx tsx scripts/verify-translation-sandwich.ts
```

(No `LAYER1_REPLAY_CACHE=1` — fresh extraction for all six L1 caches AND fresh L3 generation for F1+F2 + F7/F8/F9.)

**Important: this re-extracts F1–F6 Layer 1 schemas with the M1-CP4e tightened category 12 prompt.** Sonnet may produce different outputs than the M1-CP4e-A cache (which was extracted at M1-CP4e-A under the same tightened prompt). Drift is possible; the harness recalibrations from M1-CP4e-A should accommodate it. Total cost: ~$0.50.

Expected: 220+ checks pass. F7's `element_fusion_detected.fused === true`; F8 SCOPE_AMBIGUITY fires at Position 6; F9 TEMPORAL_AMBIGUITY fires at Position 2. F1–F6 maintain the recalibrated baseline.

If F1–F4 unexpectedly fire ELEMENT_FUSION (over-firing despite the tightened prompt): this is a third PR5 recurrence and triggers the harness assertion strategy refactor (M1-CP4f scope). For M1-CP4e-B specifically, the response is to either tighten the prompt further OR move the affected fixtures' expectations to allow Tier 1 firing. Founder decides.

If F7 does NOT fire ELEMENT_FUSION (under-firing): tighten the system prompt's POSITIVE INDICATOR with a stronger F7-style worked example. Re-extract.
...
- F1+F2 layer3 prose caches regenerated alongside F7/F8/F9 (filling gap from M1-CP4e-A's recalibration).
...
The entry should explicitly note whether PR5 watch-status promoted to permanent KG entry (third recurrence observed) or stayed at watch.

### Step 9 — Session close (full form per cache)

Pattern: per `/adopted/standing-protocol-cache.md` §"Critical-risk sessions". Full form. Names M1-CP4f (parallel-run.ts orchestrator follow-up + comparison-table baseline reset + per-layer cost capture + admin/test-reason fixtures + harness assertion strategy refactor per the PR5 watch-status note from M1-CP4e-A) as the next session.
...
| Step 1 — confirm M1-CP4e-A deployed + 207/207 baseline | 5-10 min |
| Step 2 — generate + provision env var | 15-25 min (founder Vercel-side action) |
| Step 3 — add F7/F8/F9 fixtures | 15-25 min |
| Step 4 — real-Sonnet harness run + cache F7/F8/F9 + regenerate F1+F2 layer3 | 25-40 min (LLM calls; possible iteration if calibration surprises surface) |
...
- **M1-CP4f (Elevated)** — parallel-run.ts orchestrator follow-up + comparison-table baseline reset filtering pre-Tier-1 rows + per-layer cost capture (M1-CP5 first-pass Open Q1) + admin/test-reason fixtures + JSON export button + (per M1-CP4e-A's PR5 watch-status note) harness assertion strategy refactor to separate structural assertions from content assertions.
...
If M1-CP4e-B surfaces calibration issues at Step 4 (real-Sonnet harness run): the session pauses with prompt-tightening iteration or fixture-expectation adjustment. PR5 watch-status promotion to permanent KG entry triggers a small follow-up to operations/knowledge-gaps.md.
