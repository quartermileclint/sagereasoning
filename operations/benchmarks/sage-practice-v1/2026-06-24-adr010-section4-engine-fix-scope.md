...
> **BUILT 2026-06-25 (dark, flag-gated).** This spec is now implemented behind `SUBSTRATE_PROXIMITY_DIKAIOSYNE_ENABLED` — see `2026-06-25-adr010-section4-engine-fix-build-results.md`. **Reconciliation of §4.2 below:** the repo-only battery now DOES probe the **LOCUS-1** over-strictness direction (the OS1/OS2/OS3 good fixtures + `printOverStrictness` — PASS 0/9); what remains for the §4 **full-sandwich (LOCUS-2)** battery is the over-strictness direction *on real extractions* (does the live Layer-1 prompt emit met-argued obligations so good actions are not over-floored — currently it does not; the prompt change is the activation prerequisite). LOCUS-1 over-strictness ≠ LOCUS-2 over-strictness. The adversarial review also surfaced a **bridge-retirement gate** (the native trigger is narrower than the §3 bridge on role-only circle-free injustices — do not retire the bridge until the LOCUS-2 battery proves coverage-equivalence; see ADR-010 §4 build record).
