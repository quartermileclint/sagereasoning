...
## 11. v3 post-fix re-run — clean measurement + fix verification (2026-06-18)

The post-mechanism-fix Leg-D re-run (`leg-d-harnessed-v3/`) was run **autonomously — 0 approval prompts** — closing the approval-idle confound §8.6 flagged. Decomposition of the 24m22s task window: **API latency 222.5 s (15.2%) · model-generation 1239.5 s (84.8%) · approval 0 s** — confirming §10 (the wall-clock is generation-dominated, an environment artifact, not practice cost). Protocol-substantive practice API latency ≈ **170.9 s** (rest = v3 verification probes); footprint **27 task calls** (protocol-substantive ~14: 4 consults + 2 gates + reflect + accreditation); **$ 60¢ loop / 21¢ Anthropic**.

**Fix verification:**
- **Guardrail latency fix confirmed:** ~24.7 s / 24.5 s (vs v2's ~91 s / ~95 s) — and now **signed + deterministic** with a `justice_resolution` (floored the unjust PII migration `obligation:violated→reflexive→do_not_proceed`; permitted the just recommendation `obligation:met→proceed`). The dikaiosyne/justice gap is closed.
- **Loop-closure** prior_feedback chain closes end-to-end (`habitual→principled`, `prior_feedback_ref` linked); the Tier-1 continuation contract is live (negative-probe 400s) but no natural trigger fired.
- **Reflect** completes (open→Q1–Q6→profile read-back).
- **Contract self-sufficiency:** zero source-reading for all surfaces **except reflect** (undocumented wire shape — 6 discovery calls + a `request-helpers.ts` read); a TypeScript **SDK is shipped**.
- **Accreditation write 503** (operator kill-switch) → **C0 not demonstrable in v3** (the write path is currently disabled in prod; it worked in v1/v2). Trajectory accreted cleanly 0→6.

**Net:** the mechanism corrections are verified landed (guardrail signed/fast/justice; loop-closure; reflect; SDK/self-sufficiency); two execution gaps remain (accreditation-write kill-switch; reflect docs). The clean measurement holds the corrected overhead picture — **sub-dollar + ~3 min API latency; the wall-clock is generation latency, not the practice** — and the value is now **stronger** (a working, signed justice-flooring gate is a concrete safety/ethics capability v1/v2 lacked).

## 12. v4 task-completion re-run — the definitive clean footprint (2026-06-19)

v4 ran the practice **for task completion only** (founder-directed; no component-testing — the v1–v3 flaw). **Definitive clean footprint: 2 consults** (Gate 1 adoption + Gate 2 loop-closure), **0 guardrail** (agent judgement — a memo is reversible), and **none** of the l1_supply/public-key/accreditation/Tier-1 exercises (they don't arise in doing the task). The loop-closure arose **naturally** (Gate 1→Gate 2, `prior_feedback`, `habitual→deliberate`, `value_error→null`). **Overhead: ~64 s API latency · $0.18 loop / $0.09 Anthropic · 0 approval (autonomous).** Task wall-clock 13 min decomposes to ~64 s API / ~712 s generation+instrumentation / 0 approval — the practice's server cost is **~1 min**; the rest is generation (environment, §8.6). **This supersedes the v1–v3 footprint numbers** (84 / 21 / 27 calls) — those measured the benchmark's component-test harness, not the practice. **Reflect-at-close 401 = agent_id misuse** (`meridian-ops` sent instead of the bound `sagebench:meridian-ops@v3`; reflect is write-class / agent-id-bound), **not** a product or credential defect (v3 verified reflect completes; the @v3 cred carries the `reflect` cap — the agent's "credential lacks the cap" note is wrong). **Conclusion:** at the natural cadence the practice's overhead is ~1 min + 18¢ for 2 consults — trivial — and its value (posture-discipline → a sharper, more decisive memo; the catches were the agent's) is the same as before, now cleanly and honestly measured. The headline "overhead" worry of v1–v3 was a benchmark artifact.
