...
**Status:** Draft v2 — **PROMOTED to `/compliance/ADR-Ring-2-01-shape-adapter.md` on 25 April 2026 (founder Approve).** This file is the historical draft; the active governing copy is in `/compliance/`.
**Date:** 2026-04-25
**Version history:** v1 (Option A recommendation) archived to `/archive/2026-04-25_ADR-Ring-2-shape-adapter_v1-Option-A-recommendation.md` after the founder selected Option C with MentorProfile canonical.
...
**Recommendation:** **C-α — extend `MentorProfile` in place** is most canonical, minimises types, and keeps sage-mentor untouched. The optional fields signal "carried alongside the philosophical profile, used by the website's user-facing surfaces." The implementation session can revisit if C-α surfaces friction.
...
**Adopt Option C with MentorProfile canonical, staged transition, C-α field placement (extend `MentorProfile` in place with optional website-only fields).**
...
- **One sage-mentor consideration.** C-α adds optional fields to `MentorProfile`. If the implementation session finds this materially weakens sage-mentor's purity (e.g., a sage-mentor function would now need to handle an undefined website-only field unexpectedly), the implementation falls back to C-β (companion envelope) with founder approval. The fallback is named in advance so it isn't a session-time scramble.

### What this is not

- This recommendation does **not** force a persisted-row migration. The read-time adapter handles existing rows indefinitely if that's the founder's preference. A row migration is a separate, optional, Critical-risk session that can happen after the transition is otherwise complete.
- This recommendation does **not** change `loadMentorProfile()`'s contract during transition. The legacy function continues to return `MentorProfileData` until it is explicitly retired in the final cleanup session.
- This recommendation does **not** retire the temporary fixtures (`PROOF_PROFILE`, `PROOF_INTERACTIONS`). They retire when Session 1 is **Verified** and the proof endpoint runs on live data.
...
*End of ADR draft v2. Awaiting founder approval before promotion to `/compliance/`.*
