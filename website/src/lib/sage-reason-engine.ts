...
export interface ReasonInput {
  ...
  /**
   * R19d mirror-principle opt-in (D-R19D-ALL-TOOLS, 2026-06-07).
   * When true, the engine appends the evaluation-tool mirror-principle block
   * (MIRROR_PRINCIPLE_EVAL) as an extra system message block. Used by the
   * engine-backed scoring routes (/api/score, /api/score-decision,
   * /api/score-social, /api/score-conversation) so they carry R19d without
   * editing the shared default prompts — which /api/reason and /api/guardrail
   * also use and which are deliberately kept mirror-free. Defaults to false →
   * byte-identical behaviour for callers that do not opt in.
   */
  applyMirrorPrinciple?: boolean
  /**
...
export async function runSageReason(params: ReasonInput): Promise<ReasonResult> {
  ...
  // R19d mirror principle (opt-in; D-R19D-ALL-TOOLS 2026-06-07). Appended as its
  // own system block only when a caller sets applyMirrorPrinciple. /api/reason and
  // /api/guardrail never set it, so their system prompt is byte-identical.
  if (params.applyMirrorPrinciple) {
    systemMessages.push({ type: 'text', text: MIRROR_PRINCIPLE_EVAL })
  }
