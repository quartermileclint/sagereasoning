...
interface WritePathProfile {
  ...
  /** The examination-mode timing distinction this write-path attests. The two
   *  discretionary API paths attest a post-decision check; only the
   *  harness-enforced path attests pre-decision framing (and only the route may
   *  select it, and only for an operator-issued harness credential). */
...
const WRITE_PATH_PROFILE: Record<CoverageWritePath, WritePathProfile> = {
  ...
  // The examination is a discretionary post-decision check (the agent chose to
  // submit after forming its judgement).
  wrapper_write: {
  ...
  // pre-decision marker (api_keys.credential_provenance — admin-mint only; a
  // consumer cannot self-issue it). coverage_status stays agent_elected (D3 — the
  // pre/post distinction is a TIMING property, NOT coverage breadth; we do not
  // repurpose 'continuous'). The window is still self-reported (same write
  // surface) — the harness attests TIMING via examination_mode, not the window.
  harness_enforced: {
...
export function composeK1InitialCoverage(
  ...
  return {
    ...
    // separate field carries the timing. Whether it is WRITTEN to the DB is the
    // store's flag-gated decision (byte-identical when the flag is off).
