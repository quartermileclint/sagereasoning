/**
 * PORTED — verbatim mirror of /trust-layer/grade-engine/grade-transition-engine.ts
 *
 * Source of truth: /trust-layer/grade-engine/grade-transition-engine.ts
 *   (built 3 April 2026).
 ...
 *
 * Component 4 (trajectory awareness): the ATL Wrapper drives
 * evaluateGradeTransition off the WindowSnapshot + its carried AccreditationRecord.
 *
 ...
 *   They are module-private — never exported, never returned in any result. A
 *   TransitionResult exposes only the (R4-respecting) AccreditationRecord, a
 *   plain-English evidence_summary, and an explanation string.
 *
 * KEEP IN SYNC: if /trust-layer/grade-engine/grade-transition-engine.ts changes,
