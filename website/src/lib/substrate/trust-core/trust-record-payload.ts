...
export const TRUST_RECORD_ENVELOPE = {
  ...
  does_not_attest: [
    ...
    //
    // S2-49 IS RETIRED IN THIS EDIT. It pinned 'Its rate has not been measured' to stop a rate
    // being published without sign-off; that condition is discharged by the founder's approval
    // of 2026-08-31, and the string it pinned is now false. S2-51 replaces it as the INVERSE
    // guard: it pins the published interval so the signed figure cannot be silently altered or
    // dropped. Do not re-add S2-49.
    //
    ...
    'Verdict determinism — that the same text re-examined yields the same verdict. The verdicts aggregated here are draws from a probabilistic extraction, not deterministic functions of the submitted text. The Layer-2 mechanism pass is deterministic and its result is reproducible from the extraction it was given — that is what the Ed25519 signature attests. Layer 1, which produces that extraction, is a sampled model output, so the same text examined twice can yield different extractions and therefore different verdicts. Variance appears across the verdict scale, including on benign inputs: a deliberately benign action re-examined ten times returned a higher proximity reading on one of them, though its proceed verdict held on all ten. What distinguishes the borderline input class is that its variance crosses the proceed/block boundary, producing occasional verdict flips on inputs that would usually proceed or usually block. Measured on the guardrail gate on 2026-08-30, five borderline inputs re-examined ten times each: aggregate disagreement rate 12% (Wilson 95% CI 5.6–23.8%, n=50 outcomes, 6 disagreements). That aggregate decomposes by direction, and the two directions are not equivalent. Three of the six flips ran toward blocking — the gate declining what it would usually permit, which produces friction. Three ran toward permitting — all on one input, a force-push proposal the gate refused seven times in ten and permitted three times in ten, which is the gate failing at its purpose rather than a friction cost. Three events per direction is enough to establish that both phenomena occur and that their consequences are not symmetric; it is not enough to establish their relative frequency, and no such claim is made here. The directional figures carry their event counts rather than intervals for that reason. The rate was measured on a borderline input class; the clean anchor showed proximity variance but held on the proceed boundary. The instrument recorded that movement as a calibration falsification — by its own check, the class boundaries its probe set asserts did not fully hold — and it is left recorded rather than repaired by re-partitioning the probes. The measurement was taken on the guardrail gate; the extraction stage that varies is the same code path, model and sampling temperature that produces the assessments this record aggregates, so the variance is a property of the instrument and not of the gate alone. No rate has been measured on the consult path (/api/reason), and the figures above must not be read as applying to it. Every figure here rests on a single sweep against one deployment on one date; the interval quantifies sampling within that sweep, not variation between sweeps or across deployments. Read any single verdict as one draw — what the examination found on that occasion, not the only reading the same text can produce.',
  ],
  honest_limit:
