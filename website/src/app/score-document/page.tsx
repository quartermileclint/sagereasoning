          ...
          <div className="space-y-4">
            <div>
              <label htmlFor="title" className="block font-body text-sage-700 text-sm mb-1">
                Document Title <span className="text-sage-600">(optional)</span>
              </label>
              <input
                id="title"
                type="text"
            ...
            {loading && (
              <p className="font-body text-sage-600 text-sm text-center animate-pulse">
        ...
        {result && (
          <div className="space-y-6">
            ...
            <div className={`rounded-xl border-2 p-8 ${PROXIMITY_BG[result.katorthoma_proximity] || 'bg-gray-50 border-gray-200'}`}>
              <div className="text-center mb-6">
                ...
                <div className="mt-3">
                  ...
                  <p className="font-body text-sage-600 text-sm mt-1">Right Action Proximity</p>
                </div>
              </div>
              ...
              <div className="text-xs font-body text-sage-600 text-center">
            ...
            <div className="bg-white border border-sage-200 rounded-lg p-6">
              <h3 className="font-display text-sm font-medium text-sage-600 uppercase tracking-wider mb-3">
                Stage 1 — Control Filter
              </h3>
              <div className="grid md:grid-cols-2 gap-4">
                <div>
                  <span className="font-body text-xs text-sage-600 block mb-2">Within Author&apos;s Control</span>
                ...
                <div>
                  <span className="font-body text-xs text-sage-600 block mb-2">Outside Author&apos;s Control</span>
                  <ul className="space-y-1">
                    {result.authorial_control.outside_control.map((item, i) => (
                      <li key={i} className="font-body text-sm text-sage-600">• {item}</li>
                    ))}
                  </ul>
                </div>
              </div>
            ...
            <div className="bg-white border border-sage-200 rounded-lg p-6">
              ...
              {result.passions_detected.authorial_passions.length > 0 && (
                <div className="mb-4">
                  <span className="font-body text-xs text-sage-600 block mb-2">In the Author</span>
              ...
              {result.passions_detected.reader_triggered_passions.length > 0 && (
                <div className="mb-4">
                  <span className="font-body text-xs text-sage-600 block mb-2">Triggered in Readers</span>
              ...
              {result.passions_detected.false_judgements.length > 0 && (
                <div>
                  <span className="font-body text-xs text-sage-600 block mb-2">False Judgements Identified</span>
                  <ul className="space-y-1">
              ...
              {result.passions_detected.authorial_passions.length === 0 &&
               result.passions_detected.reader_triggered_passions.length === 0 && (
                <p className="font-body text-sm text-sage-600 italic">No significant passions detected.</p>
              )}
            </div>
            ...
            {/* Disclaimer (R3) */}
            <p className="font-body text-xs text-sage-600 text-center italic">
              {DOCUMENT_EVALUATIVE_DISCLAIMER}
            </p>
      ...
      {passions.map((p, i) => (
        <div key={i} className="bg-sage-50 rounded p-3">
          <div className="flex items-center gap-2 mb-1">
            ...
            {p.sub_species && (
              <span className="font-body text-xs text-sage-600">({p.sub_species})</span>
            )}
          </div>
          <p className="font-body text-xs text-sage-600 mb-1">
            <span className="text-sage-600">Evidence:</span> {p.evidence}
          </p>
          <p className="font-body text-xs text-sage-600">
            <span className="text-sage-600">False judgement:</span> {p.false_judgement}
          </p>
        </div>
      ))}
    </div>
