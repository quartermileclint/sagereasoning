    ...
    try {
      if (storageMode === 'cloud') {
        ...
        if (!res.ok) {
          ...
          setErrorMsg(err.error || 'Something went wrong saving your entry. Please try again.')
          setSubmitting(false)
          return
        }
      } else {
      ...
      setErrorMsg('Something went wrong saving your entry. Please try again.')
    }
    setSubmitting(false)
  }
  ...
  if (loading) {
    return (
      <div className="max-w-4xl mx-auto px-6 py-16">
        <div className="text-center text-sage-600 font-body">Loading your journal...</div>
      </div>
    )
  }
    ...
    return (
      <div className="max-w-3xl mx-auto px-6 py-16">
        ...
        <div className="bg-white border border-sage-200 rounded-lg p-8 mb-8">
          ...
          <p className="font-body text-xs text-sage-600 mt-6 text-center">
          ...
          <div className="space-y-3">
            {PHASES.map(phase => (
              <div key={phase.number} className="flex items-start gap-3">
                <span className="font-display text-sm font-medium text-sage-600 w-16 shrink-0">
                  Phase {phase.number}
                </span>
                <div>
                  <span className="font-display text-sm font-medium text-sage-800">{phase.title}</span>
                  <span className="font-body text-sm text-sage-600 ml-2">({phase.days})</span>
                </div>
              </div>
            ))}
          </div>
          <p className="font-body text-sm text-sage-600 mt-4">
    ...
    <div className="max-w-4xl mx-auto px-6 py-12">
      ...
      <div className="mb-8">
        <div className="flex justify-between items-center mb-2">
          ...
          <span className="font-body text-sm text-sage-600">{completionPercent}%</span>
        </div>
            ...
            return (
              <div key={p.number} className="mb-4">
                <div className="font-display text-sm font-medium text-sage-700 mb-2">
                  ...
                  <span className="font-body text-xs text-sage-600 ml-2">Days {p.days}</span>
                </div>
                    ...
                    return (
                      <button
                        ...
                        className={`w-8 h-8 rounded text-xs font-body flex items-center justify-center transition-colors ${
                            ...
                            : d <= currentDay
                            ? 'bg-sage-100 text-sage-600 hover:bg-sage-200 cursor-pointer'
                            : 'bg-sage-50 text-sage-600 cursor-not-allowed'
                        }`}
          ...
          <div className="bg-sage-50 border border-sage-200 rounded-lg p-6 max-w-lg mx-auto mb-6 text-left">
            ...
            <div className="space-y-3">
              <div>
                ...
                <p className="font-body text-xs text-sage-600">Are fewer passions operative? Are they less intense than when you started?</p>
              </div>
              <div>
                <span className="font-display text-xs font-medium text-sage-600">2. Quality of Judgement</span>
                <p className="font-body text-xs text-sage-600">Is your understanding of what is genuinely good, bad, and indifferent more accurate?</p>
              </div>
              <div>
                <span className="font-display text-xs font-medium text-sage-600">3. Stability of Disposition</span>
                <p className="font-body text-xs text-sage-600">Does your commitment to virtue hold under pressure? How quickly do you recover after setbacks?</p>
              </div>
              <div>
                <span className="font-display text-xs font-medium text-sage-600">4. Expanding Circles of Concern</span>
                <p className="font-body text-xs text-sage-600">Are you increasingly taking account of obligations beyond yourself?</p>
              </div>
            </div>
          </div>
          ...
          {isViewingPast && pastEntry ? (
            <div className="bg-white border border-sage-200 rounded-lg p-6 mb-6">
              <div className="flex items-center justify-between mb-3">
                <span className="font-display text-sm font-medium text-sage-600">Your reflection</span>
                <button
              ...
              <div className="mt-5 pt-4 border-t border-sage-100 flex flex-col items-center gap-2">
                ...
                <p className="font-body text-xs text-sage-600 text-center max-w-xs">
            ...
            <div className="bg-white border border-sage-200 rounded-lg p-6 mb-6">
              {submitted ? (
                ...
                <>
                  ...
                  {errorMsg && (
                    <p role="alert" className="mt-3 font-body text-sm text-red-700 bg-red-50 border border-red-200 rounded p-3">
                      {errorMsg}
                    </p>
                  )}
                </>
      ...
      {!allComplete && (
        <div className="flex justify-between items-center text-sm font-body">
          <button
            ...
            className="text-sage-600 hover:text-sage-600 disabled:opacity-30 disabled:cursor-not-allowed"
          >
            &larr; Previous day
          </button>
          <button
