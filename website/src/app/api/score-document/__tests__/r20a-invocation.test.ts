 ...
 * for /api/score-document. Created 2026-09-05 by Session 3B (Group 2, item
 * 8) of the perimeter-ordering audit (operations/count-discipline-2026-09/
 * 2026-09-05-r20a-perimeter-ordering-AUDIT.md §6), executing the 2026-09-06
 * mentor ruling.
 *
 * Run: npx tsx src/app/api/score-document/__tests__/r20a-invocation.test.ts
 ...
 * field is `text` at the D bound (30,000 — the largest raw input any
 * perimeter member sends to the classifier, so the cap matters most here);
 * the first post-guard anchors are `const trimmed` (which the 20-word
 * minimum — already after the check — reads) and the RAG block loader.
 */
...
const MAX_TEXT_RE = new RegExp(`validateTextLength\\(\\s*text\\s*,\\s*${QUOTED}\\s*,\\s*TEXT_LIMITS\\.document\\s*\\)`)
...
const CAP_RE = /const\s+screenedText\s*=\s*text\.slice\(\s*0\s*,\s*TEXT_LIMITS\.document\s*\)/
const SUBJECT_RE = /detectDistressTwoStage\s*\(\s*screenedText\s*\)/
const RAW_SUBJECT_RE = /detectDistressTwoStage\s*\(\s*text(?:\.slice\([^)]*\))?\s*\)/
const TRIM_RE = /const\s+trimmed\s*=\s*text\.trim\(\)/
const WORD_MIN_RE = /wordCount\s*<\s*20/
...
const wordMinIdx = codeIndexAfter(code, WORD_MIN_RE, checkIdx)
...
expectTrue(
  'MAX-1 the text maximum guard (document bound) follows the structural END of the redirect-return block',
  maxTextIdx > -1 && block.endIdx > -1 && maxTextIdx > block.endIdx,
  `max=${maxTextIdx} blockEnd=${block.endIdx}`,
)
expectTrue(
  'MAX-2 the maximum still precedes `const trimmed`, the 20-word minimum and the first RAG/context load (order, not existence)',
  maxTextIdx > -1 && trimIdx > -1 && wordMinIdx > -1 && contextLoadIdx > -1 &&
    maxTextIdx < trimIdx && maxTextIdx < wordMinIdx && maxTextIdx < contextLoadIdx,
  `max=${maxTextIdx} trim=${trimIdx} wordMin=${wordMinIdx} load=${contextLoadIdx}`,
)
expectTrue(
...
expectTrue(
  'CAP-1 the classifier receives screenedText (= text.slice(0, TEXT_LIMITS.document)), defined exactly once after the presence check and before the check, and never the raw field',
...
expectTrue(
  'CAP-2 the cap key (TEXT_LIMITS.document) is the SAME key the moved text guard enforces (the cap equals the bound)',
  /validateTextLength\(\s*text\s*,[^)]*TEXT_LIMITS\.document\s*\)/.test(code) && CAP_RE.test(code),
)
expectTrue(
  'CAP-3 TEXT_LIMITS.document is the audit\'s D bound (30,000) as read from security.ts source',
  LIMITS.document === 30000,
