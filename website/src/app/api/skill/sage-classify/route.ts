...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (isR20aGapClosureEnabled()) {
      const subject = composeDistressSubject([input, context])
      // PR19 (2026-08-18 fold, extended 2026-08-22): skip the classifier on an
      // empty subject — missing/empty `input`/`context` have no distress to
      // detect, and calling it anyway pays for a real billed Haiku call
      // before the input-required check below fires.
