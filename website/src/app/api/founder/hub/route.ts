...
// ADR-Ring-2-01 Session 3e (26 April 2026): migrated to the canonical
// loader (then named `loadMentorProfileCanonical`; renamed to
// `loadMentorProfile` at Session 5 close, 26 April 2026, when the legacy
// loader was retired). The loaded profile flows only into
// `getRecentInteractionsAsSignals()` — migrated alongside in
...
// Session Opening Protocol pointer — prepended to session prompts surfaced to the founder.
// Call sites:
...
//      consumed the raw synthesis, so the pointer does not pollute mentor review input.
...
// Design note: this constant was a verbose 8-element extract until 2026-04-25; shortened to a
// one-line pointer under DD-2026-04-25-05 because the agent already reads the file at session open.
// Prior verbose extract preserved at /archive/2026-04-24_session-opening-protocol-hub-extract_applied.md.
const SESSION_OPENING_PROTOCOL_EXTRACT = `Governing frame: /adopted/session-opening-protocol.md
...
${mentorKB}`, cache_control: { type: 'ephemeral' } },
    ...
    // Four per-agent wrappers (April 2026 internal set). Each carries the
    // shared preamble plus its domain-specific reasoning upgrades. If the
...
${brainContext}`
    ...
    systemBlocks = [
      { type: 'text', text: primaryText, cache_control: { type: 'ephemeral' } },
      ...(stoicContext ? [{ type: 'text' as const, text: stoicContext }] : []),
    ]
  }
  ...
  // ADR-Ring-2-01 Session 3e: uses the canonical `loadMentorProfile()` —
  // `storedProfile.profile` is the canonical MentorProfile shape, which
