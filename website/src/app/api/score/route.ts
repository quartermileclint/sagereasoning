...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // The MAXIMUM-length guards on `action` and `context` (provenance aeadbd1
    ...
    const screenedAction = action.slice(0, TEXT_LIMITS.short)
    const gate = await enforceDistressCheck(detectDistressTwoStage(screenedAction))
    ...
    // `action` / `context` MAXIMUM length — MOVED here 2026-09-05 (Session 3B,
    // Group 2 of the perimeter-ordering audit, §6 item 8) under the 2026-09-06
    // ruling (D-MENTOR-RULING-R20A-LENGTH-GUARD-ORDERING-ADOPTED-2026-09-06).
    // A long distressed action now reaches the check above (capped at this
    ...
    // order are unchanged, and both still precede domainContext and every
