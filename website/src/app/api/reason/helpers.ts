/**
 * helpers.ts — pure functions used by the /api/reason route's RAG wiring.
 *
 * Sub-session E1 (2026-05-04): wires D6 + D7 into /api/reason quick-depth
 * (and forward-compatible for standard + deep) via Pattern A2 (structured
 * passages to engine).
 *
 * Extracted to a sibling file so the verification harness at
 * /website/scripts/verify-reason-rag.ts can import + test them
