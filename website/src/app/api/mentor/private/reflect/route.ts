...
// A18c (R20b): deterministic framework-dependence detector. Pure, read-only,
// no LLM. Consumed only on this single-endpoint proof (PR1) until Verified.
import {
  detectFrameworkDependence,
  DEPENDENCE_DEFAULTS,
  type DependenceSignal,
} from '@/lib/r20b-dependence'
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // (provenance 9fe99f0 2026-04-11, file creation) used to sit HERE, before
    ...
    // r20a-perimeter-ordering-AUDIT.md §6, item 5 — same shape as
    // /api/reflect, moved together) under the same binding ruling the minimum
    // moved under. See the guards' new site below.
    ...
    // screened — before this move it was not read at all (a bare 400). A
    // NON-STRING `how_i_responded` is coerced with String() (the template
    // literal's own ToString) and THEN sliced, so the bound holds for every
    // value (PR19 fold, 2026-09-06 — see /api/reflect's note). COST,
    // disclosed: an oversized regex-silent field now reaches stage 2 (Haiku)
    ...
    // NOT EXISTENCE: values, messages and status are unchanged, and both
    // guards still precede the minimum below, every context load and the LLM
    ...
    // ─── R20b — FRAMEWORK-DEPENDENCE DETECTION (A18c) ────────────────────
    // Flag-gated; OFF by default. When R20B_INDEPENDENCE_COACHING_ENABLED is
    // 'true', read this practitioner's recent interaction frequency +
    // triviality and, if the dependence pattern is present, surface an
    // independence-coaching instruction in the mentor's user message (block
    // injected below during userMessage assembly). Deterministic, read-only,
    // synchronous (PR3), non-blocking (try/catch → null). Runs AFTER the R20a
    // distress gate; does NOT touch the distress classifier / Zone 2-3 /
    // wrapper (PR6 boundary — ADR 2026-06-07). When the flag is unset the whole
    // block is skipped and the request is byte-identical to pre-A18c.
    let dependenceSignal: DependenceSignal | null = null
    if (process.env.R20B_INDEPENDENCE_COACHING_ENABLED === 'true') {
      try {
        const { data: depProfileRow, error: depProfileErr } = await supabaseAdmin
          .from('mentor_profiles')
          .select('id')
          .eq('user_id', auth.user.id)
          .single()
        if (depProfileErr) {
          console.error(
            '[private/reflect] R20b profile lookup error (non-blocking):',
            depProfileErr,
          )
        }
        const depProfileId = (depProfileRow as { id?: string } | null)?.id ?? null
        if (depProfileId) {
          const depInteractions: InteractionRecord[] =
            await loadMentorInteractionsAsRecords(
              depProfileId,
              PRIVATE_MENTOR_HUB as InteractionsHubId,
              { windowDays: DEPENDENCE_DEFAULTS.windowDays, limit: 200 },
            )
          dependenceSignal = detectFrameworkDependence(depInteractions)
        }
      } catch (depErr) {
        console.error(
          '[private/reflect] R20b dependence detection error (non-blocking):',
          depErr,
        )
        dependenceSignal = null
      }
    }
...
Score my actions and give me the sage perspective.`
    ...
    // R20b (A18c): independence-coaching instruction — only when the
    // dependence pattern is present. Diagnostic, not punitive (R6d); an honest
    // invitation toward self-reliance, never a gate (R19/R20b). When the signal
    // is absent (or the flag is off) this block is not added and the request is
    // byte-identical to pre-A18c.
    if (dependenceSignal?.present) {
      userMessage += `\n\nINDEPENDENCE SIGNAL (R20b — diagnostic, not punitive): This practitioner's recent usage shows a high-frequency, low-stakes pattern (${dependenceSignal.reason}). Per R20b, gently and warmly affirm — once, in your own voice, only if it fits naturally — that they may be ready to reason through routine decisions on their own (for example, "you're ready to reason through this yourself"). Frame it as a mark of progress and growing self-reliance, not a criticism or a limit. Do not repeat it or labour the point, and never withhold, shorten, or limit the reflection itself.`
    }
    ...
    const result = {
      ...
      // A18c (R20b): framework-dependence diagnostics. null when the flag is
      // off or detection didn't run; otherwise the DependenceSignal.
      // present=true means the independence-coaching block was injected this
      // request. Surfaced for founder verification.
      r20b_dependence: dependenceSignal,
    }
