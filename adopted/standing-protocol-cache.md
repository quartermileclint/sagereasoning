...
**Status:** Adopted 2026-05-03 under `D-PHASE-2-PASS-1-REPLAN-EFFICIENCY-REFINEMENT-2026-05-03`. **Amended 2026-05-12 under `D-CACHE-DRIFT-RESOLVED-2026-05-12`** to incorporate ST2 changes: AI signals table extended with three diagnostic-certainty rows; manifest range updated (AC1–AC13); process-rule range updated (PR1–PR16); project-instructions surface now `/adopted/project-instructions-snapshot.md` (first repo-tracked snapshot, created at ST2 adoption); cross-references updated for new ADR (J1 Character Kernel) and amended staging plan. **Amended 2026-05-14 under `D-CACHE-DRIFT-RESOLVED-2026-05-14`** to incorporate the Anthropic-native posture session changes: `/CLAUDE.md` entry-point file created at repo root for Claude Code sessions; 17 Anthropic skills installed at `/.claude/skills/anthropic/`; PR15 amended (operational discipline mandating consultation of `.claude/skills/anthropic/` + agentic-commerce-findings tracker before bespoke election). **Amended 2026-06-10 under `D-PR18-ADOPTED-CLOSE-TIME-PRODUCTION-STATE-2026-06-10`** (same-session cache update per the update discipline): process-rule range PR1–PR18; PR18 = production-state blocks are close-time artifacts. **Amended 2026-07-21 under `D-CACHE-DRIFT-RESOLVED-2026-07-21`** (same-session cache update per the update discipline, P3 of the Agent-Organization + Evidence Program): process-rule range PR1–PR19; PR19 = independent adversarial review is required (not optional) before a trust-core/predicate/fold/engine change or a live-op-consequential build plan is treated as verified, with a codified spend-limit-outage fallback and a reusable review-workflow template at `operations/review-harness/independent-review-workflow-template.md`. **Amended 2026-08-04 under `D-PR20-ADOPTED-MENTOR-BRIEF-ARCHITECTURAL-SURFACES-2026-08-04`** (same-session cache update per the update discipline, following the Stoa Q5c/Q13a floor-creation finding): process-rule range PR1–PR20; PR20 = a mentor-consultation brief on any question with an architectural consequence must name the specific existing mechanisms (one-sentence, mechanism-level facts about current behaviour) the ruling will land on, so a downstream consequence the mentor cannot see from the question alone is visible before the ruling is given, not discovered afterward by PR19 adversarial review. **Amended 2026-08-10 under `D-ARC2-SESSION1-PROCESS-ADOPTIONS-2026-08-10`** (same-session cache update per the update discipline; ARC2 session 1, discharging five process recommendations the 2026-08-01 regrounding audit raised and no session had ever put to the founder): process-rule range **PR1–PR24**. **PR19 scope-widened** to add auth/security/R20a-perimeter code and any code that deletes data (grounding: a new cron route outside PR19's original four surfaces returned three confirmed defects on independent review, including a filter-value defect that would have deleted live in-retention rows and which the author's own 47-assertion battery passed silently). **PR21** = reflect-harvest — the reflect close-turn's findings must reach the decision-log entry (write side) and be read at the next session's open (read side), because the turn fires *after* the close is written and 20 extracted reflect turns had reached no tracked artifact. **PR22** = `Model:`/`Effort:` trailers on every AI-authored commit, convention-only by founder election (re-measured at adoption: 4 of 128 commits since 2026-07-19 carried one, 3 of them same-day; the pre-commit hook is the named escalation if the rate does not improve). **PR23** = memory-first — consult the memory index before diagnosing *or writing code in* a recurring problem class, with the honest caveat that citing a memory without verifying the current instance against it discharges the letter and not the purpose. **PR24** = retention parity — a table declaring `retain_until` ships its purge and sweep wiring in the same session (grounding: `route_errors`/`throttle_events` declared it for ~3 weeks with nothing enforcing it, and `agent_hold_observations` still does). **CORRECTED 2026-08-17 (R2b), in lockstep with the same correction in `/adopted/project-instructions-snapshot.md`:** this grounding previously also named `stoa_entries`, which is factually wrong — that table has NO `retain_until` at all, deliberately, by binding mentor ruling #24/Q9 ("silent expiry is prohibited"), pinned in its migration, its store header, and an executing battery assertion. PR24 is conditional on a table DECLARING `retain_until`, so it never bound `stoa_entries`. Its erasure reachability was verified first-hand instead (owner-delete, export, and credential-erase are all wired), so there is no differently-shaped gap behind the wrong sentence. **AMENDED 2026-08-19 under `D-CACHE-DRIFT-RESOLVED-2026-08-19`:** the Consciousness and Continuity Obligation section (added 2026-08-09, above) gained a founder-directed pointer note — not part of the mentor's verbatim text, following the same discipline as the section's existing placement note — recording that a 2026-08-18 relayed addendum independently raised the obligation's second component (dispositional-stability/hexis) against current frontier RL-alignment approaches. No new rule, no R-code, no rewording of the mentor's text; see `operations/agent-circles-2026-08/2026-08-18-addendum-reinforcement-learning-assessment-verbatim.md`. **Amended 2026-08-19 under `D-MENTOR-RULINGS-OBSERVER-CONVENING-PR20-STALE-FACT-ADOPTED-2026-08-19`** (same-session cache update per the update discipline, following the curiosity/taxonomy-scoping session's Q5 stale-mechanism-fact finding): **PR20 strengthened** — any present-tense mechanism fact in a question document (a claim about the CURRENT state of a live surface, endpoint, schema, or build artefact) must be timestamp-checked by the relaying session against the decision log or codebase before the document is relayed, not only stated accurately at drafting time; an unverifiable present-tense fact is marked recorded-but-not-independently-verified, extending the existing PR20 convention to facts that have gone stale between drafting and relay, not only facts never verified in the first place. **Further amended 2026-08-19, same day, under `D-CARRY-FORWARD-REDIRECTED-STANDING-RUNNER-PR20-FURTHER-AMENDED-2026-08-19`:** the same timestamp check now applies to carry-forwards naming a target scoping session, checked at DRAFTING time (not only at relay time) — a carry-forward pointed at an already-closed session is neither voided nor dated-amended into the closed document, but redirected to whichever open-or-future session's subject matter actually fits, recorded there as a named input, not a pre-answer. **Amended 2026-08-24 under `D-CACHE-DRIFT-RESOLVED-2026-08-24`** (same-session cache update per the update discipline, following the reflections-examination arc's IS-1 encoding): process-rule range **PR1–PR25**; **PR25** = a verification claim in a code comment carries its check — resolved at the commit, from the staged diff, into either a named check with its actual scope or an explicit provisional marker; pilot scope is code comments only.
...
> **⚠ ANNOTATED 2026-09-05 (grounding session; a factual correction, not a rule change):** the *"zero actual collisions"* sentence above is now **false**. On 2026-09-05 a peer session's unscoped `git add` swept **187 lines of another session's decision-log entry** into commit `468fcf9` under an unrelated message (`operations/handoffs/founder/2026-09-05-D4-activation-and-F3prime-CLOSE.md` §8c) — the first real collision on record. Content intact; attribution wrong; history deliberately NOT rewritten on a shared checkout with peers live. The convention stands as written. **Whether this is the instance that triggers the named escalation (a pre-commit hook, or elevation to PR26) is the founder's call and has not been taken.** Two related practices, both observed and both now standing: on this shared checkout **a peer's push publishes your local commits** (the commit, not the push, is the point of no return), and **`git status` must be run whole, never truncated** (a `head -3` produced a false peer-collision alarm on 2026-09-05).
...
- Manifest rules (R0–R22, AC1–AC13, KG1–KG7) — AC9–AC13 added under ST2 2026-05-12; R21 (Website Image Use Policy) + R22 (Human Creator Protection Commitment) added 2026-08-09 under `D-WEBSITE-IMAGE-POLICY-AND-HUMAN-CREATOR-PROTECTION-ADOPTED-2026-08-09`; the Agent Task Reasoning Framework (ATRF) named framework section + the Consciousness and Continuity Obligation named open question (un-numbered — not new R-codes) added 2026-08-09 under `D-ATRF-AND-CONSCIOUSNESS-CONTINUITY-ADDED-2026-08-09`
...
- Process rules (PR1–PR25) — PR10–PR16 added under ST2 2026-05-12; PR15 amended 2026-05-14; PR17 (founder-performed operational steps walked through live, not handed off) added 2026-05-27 under `D-PR17-ADOPTED-WALKTHROUGH-2026-05-27`; PR18 (production-state blocks are close-time artifacts — rewritten only at session close, only from the decision log + that session's verified observations, always carrying an as-of date) added 2026-06-10 under `D-PR18-ADOPTED-CLOSE-TIME-PRODUCTION-STATE-2026-06-10`; PR19 (independent adversarial review is required, not optional, before a trust-core/predicate/fold/engine change or a live-op-consequential build plan is treated as verified; codifies the spend-limit-outage first-hand fallback + a mandatory-not-recommended independent re-run before downstream reliance; reusable template at `operations/review-harness/independent-review-workflow-template.md`) added 2026-07-21 under `D-PR19-ADOPTED-INDEPENDENT-REVIEW-REQUIRED-2026-07-21`; PR20 (a mentor-consultation brief on any question with an architectural consequence must name the specific existing mechanisms — one-sentence, mechanism-level facts about current behaviour — the ruling will land on, so a downstream consequence is visible before the ruling is given, not discovered afterward by PR19 adversarial review) added 2026-08-04 under `D-PR20-ADOPTED-MENTOR-BRIEF-ARCHITECTURAL-SURFACES-2026-08-04`; PR20 amended 2026-08-19 under `D-MENTOR-RULINGS-OBSERVER-CONVENING-PR20-STALE-FACT-ADOPTED-2026-08-19` to require present-tense mechanism facts be timestamp-checked at relay, not only stated accurately at drafting, and further amended the same day under `D-CARRY-FORWARD-REDIRECTED-STANDING-RUNNER-PR20-FURTHER-AMENDED-2026-08-19` to extend the same check to carry-forwards naming a target session, checked at drafting time; **PR19 scope-widened (auth/security/R20a-perimeter + any data-deleting code) and PR21 (reflect-harvest), PR22 (model+effort commit trailers, convention-only), PR23 (memory-first before diagnosing *or writing* in a recurring class), PR24 (retention parity — a `retain_until` table ships its sweep in the same session) added 2026-08-10 under `D-ARC2-SESSION1-PROCESS-ADOPTIONS-2026-08-10`**; **PR25 (a verification claim in a code comment carries its check — any assertion that a property has been checked or holds is resolved BEFORE THE COMMIT that ships it into either the completed-check branch, naming the check actually run *and the scope it actually covered*, or an explicit `UNVERIFIED:`/`ASSUMED:` provisional marker naming what would establish it; the named source is the staged diff, grepped for the claim vocabulary; convention-only at adoption per PR22's posture, with a pre-commit grep as the named escalation; pilot scope is code comments ONLY, the general artifact case deliberately not taken per PR1's discipline) added 2026-08-24 under `D-PR25-ADOPTED-VERIFICATION-CLAIM-CARRIES-ITS-CHECK-2026-08-24`, encoding the reflections examination's IS-1 finding on the mentor's ruling that a disposition failing in the presence of its own articulation needs a structural anchor — with the ruling's own caveat carried, not dropped: rules are scaffolding for *prosoche*, not a substitute for it**
...
- `/manifest.md` — full manifest (R0–R22, AC1–AC13, KG1–KG7; AC9-AC13 added under ST2 2026-05-12; R17g/h/i + R18e added; R18f + R19e added 2026-05-23; R20a perimeter potential-broadening placeholder; R21 Website Image Use Policy + R22 Human Creator Protection Commitment added 2026-08-09; the Agent Task Reasoning Framework (ATRF) named framework section + the Consciousness and Continuity Obligation named open question added 2026-08-09 — mentor-directed, applied verbatim, neither a new R-code)
