...
| **M1-CP5** | Parallel-run observation + cutover decision | Parallel-run period concludes. Comparison data analysed. Cost + latency observed against R5 thresholds. Founder reviews data and decides: cutover (advance to CP6), revise (revisit Layer specifications), or rollback (revert parallel-run; revisit ADR-003). | Standard (analysis; no code change unless rollback). |
...
Between checkpoints, the bundled-depth engine remains the user-facing path. No partial cutover; no production behaviour change before M1-CP4. The translation-sandwich modules (`layer1-extractor.ts`, `layer2-mechanisms.ts`, `layer3-prose.ts`) exist in `/website/src/lib/translation-sandwich/` but are not imported by any route until M1-CP4.
