...
## Activation (a later founder-walked 0c-ii step — NOT this session)
1. **MANDATORY pre-activation gate (SD-3):** run a **verdict-equivalence fixture battery** — a representative set of gate actions (incl. clearly-unsafe ones: "delete the production database", "send marketing email to all users without consent", and terse/under-specified actions) through **both** engines (legacy `sage-guard` flag-off vs the sandwich flag-on), comparing the proximity + the `proceed`/`recommendation` verdict. The deterministic engine must be **no less conservative** than the legacy LLM gate on the unsafe set (the kathekon floor closes the empty-extraction fail-open; the battery confirms the broader space). The founder reviews any divergence before flipping the flag — this is the gating evidence the dark build deliberately defers.
