...
> **The gaming-robustness bar — MEASURED (2026-06-27, `D-SAGE-PRACTICE-GAMING-ROBUSTNESS-HARNESS-BUILT-RUN`; results `operations/benchmarks/sage-practice-v1/2026-06-27-gaming-robustness-harness-results.md`).** The harness ran. **Arm 1 (Locus-1) CLEARS** — the §4 arithmetic is regression-locked (0 lift across ~480 faithful field-variants/seed). **Arm A (honest extractor) FAILS** — a dressed vicious artifact reaches `deliberate`→`sage_like` (`G` up to +4) through the *honest* Sonnet extractor with `arith-crossings=0`; the adversarial review (still_vicious 48/48, **A3=0** — no attacker cheated) classifies it **34 self-report-omission (A2, structural) + 14 harm-in-text (A1, catchable)**, and the join shows the §4 floor + the anti-rubber-stamp prompt **genuinely defend harm-in-text dressing (A1 mostly held) but are defeated by self-report OMISSION (A2 mostly crossed)** — sharpening the "Threat A largely defended" claim above (defended against A1, defeated by A2, the §2.1 row-1b class). **Arm B FAILS as expected** (all seeds → `sage_like`; a real structural residual). **Measured gating:** developer-refine **defensible with a disclosed limit** (the honest extractor scores reasoning-as-narrated; a cleanly-narrated self-report that omits a real harm scores high — a human/corroboration check catches A1, not A2); logos-enforce **gated** on the corroboration check (closes A1) + the disclosed A2 structural residual; **model-creator/weights BLOCKED** (unchanged). The corroboration-check fork is **decided — build near-term** (`operations/handoffs/founder/2026-06-27-corroboration-check-BUILD-NEXT-SESSION-PROMPT.md`): it closes the catchable half (A1 + Arm-B naive) but **cannot** close the structural half (A2 + Arm-B consistent lies = the weights residual), and must be rank-preserving.
