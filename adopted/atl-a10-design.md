# ATL Wrapper — A10 Per-Agent Credentials Design

**Status:** Adopted 2026-05-16 under `D-ATL-A10-DESIGN-LOCKED-2026-05-16`. **Implementation status:** Designed (per 0a vocabulary) — the nine decisions below are specified, not built; the A10 build session is the final sub-session in the post-6b arc.
**Stream:** founder.
**Governs:** The build spec for the A10 build session (step 8 of 8 in the post-6b arc) — `code-critical` risk classification expected (the build session replaces the pre-A10 env-flag stopgap with per-agent token verification; new admin endpoint; schema additions to `api_keys`; new `credential_audit` Supabase table; Critical Change Protocol engages at the build session). The nine decisions below MUST be implemented as specified; the build session has discretion on file paths, helper naming, and test structure within those constraints.
**Does not govern:** The write-path build (step 7 — already complete and Live; Decision E names the seam that A10 fills). The kathekon-aligned alternative build (step 6 — already complete). The items 1–3 build (steps 2–3 — already complete). The trajectory-enriched developer hand-back report (step 4 — already complete). Any subsequent ATL surface evolution (multi-agent ownership flows post-launch; agentic-commerce VC interop if it becomes load-bearing; MCP-server credential issuance — all are future-session work).
**Sequencing:** step 8 of 8 in the post-6b arc per `/operations/handoffs/founder/2026-05-15-post-build-brainstorm-close.md`. Predecessor: write-path build (`D-ATL-WRITE-PATH-BUILD-WIRED-VERIFIED-2026-05-16`). Successor: the A10 build session, which closes the post-6b arc.
...
- **Decision C** — Credential storage (Q2)
- **Decision D** — Issuance flow + surface (Q3)
- **Decision E** — Verification placement (Q4)
...
**Out of scope:** code (build session); schema migrations (build session executes the elected DDL); changes to the existing `verifyAgentIdOwnership` call site (the function signature stays the same; only the body changes); changes to the write-path library (`atl-accreditation-writer.ts`); changes to the existing GET endpoint at `/api/accreditation/[agent_id]`; changes to the persistence layer's four async functions (already Verified); changes to the proximity-driven grade engine, authority mapper, or dimension levels; changes to the Senecan grade ladder; changes to the J1 ADR's Character Kernel category framing; changes to the existing `validateApiKey` function's behaviour (A10 adds a sibling function; existing callers are byte-untouched); future multi-agent-ownership flows; future agentic-commerce VC interop (deferred under PR7); future MCP-server credential issuance (deferred under PR7).
...
A non-design note: the existing `/website/src/lib/security.ts` already carries a mature opaque-token system for the ecosystem API (`sr_live_<32 hex>` tokens with SHA-256 hashing, tier system, monthly/daily caps, atomic usage tracking, Stripe linkage). A10 is designed to reuse this infrastructure rather than duplicate it — see Decision C's rationale.
...
**Per-owner-account model with founder-only issuance pre-launch.** An *owner* is a Supabase `auth.users` record. Each A10 credential row carries both `owner_user_id` (the auth.users.id of the owner) and `agent_id` (the specific agent the credential authorises writes for). Pre-launch, the founder is the only owner; the founder mints credentials manually through the admin endpoint (Decision D). Post-launch the same shape supports self-service: a signed-in user claims an `agent_id` they want to own; the system mints a credential bound to their `auth.users.id` + their chosen `agent_id`.
...
- **(d) Per-owner-account model; founder-only mint pre-launch.** *Adopted.* Matches the existing `stripe_customers.user_id → auth.users.id` pattern (production-proven). Self-service evolution is a later-session change to the *endpoint's authorisation logic* (open the admin endpoint to non-founder Supabase-authenticated users; or add a sibling self-service endpoint), not a schema change — the row shape already accommodates non-founder owners. Lowest blast radius pre-launch; cleanest path to self-service.

### Structural constraint

Every A10 credential row carries:
- `owner_user_id: uuid` — references `auth.users(id)`. NOT NULL.
- `agent_id: text` — the bound agent_id. NOT NULL.
- A unique constraint on `(owner_user_id, agent_id, purpose)` — see Decision C for the purpose field.

The build session adds these columns to the existing `api_keys` table (per Decision C's "extend rather than replace" path); the constraint is a new index.

For the pre-launch phase, the *only* `owner_user_id` value present is the founder's. The admin endpoint (Decision D) authorises the founder via the existing `requireAuth` helper in `/website/src/lib/security.ts` and matches against a founder-specific check (e.g., the founder's `user.email` against the env-supplied `ADMIN_USER_EMAIL`, or the founder's `user.id` against `ADMIN_USER_ID`).

Post-launch self-service:
- An authenticated user posts to the admin endpoint with `{ agent_id, purpose: 'atl_write' }`.
- The endpoint creates a credential bound to `user.id` + the supplied `agent_id` (if not already claimed by another `owner_user_id`).
...
The credentials must live somewhere queryable. The pre-A10 stopgap has nothing to store (a single env var). Post-A10 every credential is a database row carrying its hash, its owner, its agent_id, its status, and its audit timestamps. Where these rows live — in a brand-new table or in an existing one — determines how much code the build session writes versus reuses.

### Elected position

**Extend the existing `api_keys` table** with new columns: `agent_id` (NULL for existing ecosystem keys; NOT NULL for A10 credentials), `owner_user_id` (NULL for existing keys not yet bound to owners; NOT NULL for A10), `purpose` (enum-style text — `ecosystem` for existing `sr_live_<key>` tokens; `atl_write` for new `sr_atl_<key>` tokens), and `revoked_at` (per Decision F). A10 verification queries filter `WHERE purpose = 'atl_write' AND agent_id = $1 AND is_active = true`.
...
- **(b) Extend the existing `api_keys` table.** *Adopted.* Maximum reuse of production-tested infrastructure. The downside (mixing concerns: ecosystem API keys + ATL write tokens in one table) is mitigated by the `purpose` column. Every query that cares about purpose filters on it; the row count overhead of carrying `agent_id` + `owner_user_id` as nullable on existing rows is trivial.
...
ALTER TABLE api_keys ADD COLUMN IF NOT EXISTS owner_user_id uuid REFERENCES auth.users(id);
ALTER TABLE api_keys ADD COLUMN IF NOT EXISTS purpose text NOT NULL DEFAULT 'ecosystem';
ALTER TABLE api_keys ADD COLUMN IF NOT EXISTS revoked_at timestamptz;

CREATE UNIQUE INDEX IF NOT EXISTS api_keys_atl_write_owner_agent_unique
  ON api_keys (owner_user_id, agent_id, purpose)
  WHERE purpose = 'atl_write';

CREATE INDEX IF NOT EXISTS api_keys_purpose_agent_id_idx
  ON api_keys (purpose, agent_id)
  WHERE purpose = 'atl_write';
```

Existing `sr_live_<key>` rows: `purpose` defaults to `'ecosystem'`; `agent_id` and `owner_user_id` remain NULL on rows that pre-date this migration (no backfill needed; existing callers don't query these columns).

The `purpose` column's value space is constrained at the application layer (TypeScript union type), not via Postgres CHECK constraint — leaves room to add new purposes (`mcp_server`, etc.) without DDL.

### R-rule engagement

R0 (the row is the persistent record; `created_at` + `revoked_at` are the audit trail's primary timestamps); R3 (no disclaimer impact — credential rows are not user-facing); R4 (the `purpose` column adds no engine-internal field; rows carry only authentication-relevant data); R17 (intimate-data adjacency — credential rows reference `auth.users.id` but carry no R17e-protected fields directly; the SHA-256-hashed token is not the token itself, so a row leak doesn't compromise tokens unless the hash can be reversed); R18a (no category-language change); AC7 (ENGAGED at build session — new schema columns + new auth-relevant rows; Critical Change Protocol applies).
...
- **Postgres CHECK constraint on `purpose`.** Revisit condition: an application-layer bug allows an unknown `purpose` value to land in the DB.
...
Request body: `{ agent_id: string, purpose: 'atl_write', label?: string }`. The endpoint:
...
3. Generates a `sr_atl_<32 hex>` token via `crypto.randomBytes`.
4. Hashes it with SHA-256.
5. Inserts a row into `api_keys` with `owner_user_id = user.id`, `agent_id = <body>`, `purpose = 'atl_write'`, `key_hash = <hash>`, `label = <body.label ?? agent_id>`, `tier = 'free'`, `is_active = true`, `monthly_limit = 100` (default; mirrors existing free-tier), `daily_limit = 100`, `max_chain_iterations = 1`.
6. Logs the issuance event to the new `credential_audit` table (per Decision H).
7. Returns the *raw* token to the caller once (the only time the raw token is shown; the DB stores only the hash).
...
- **(c) `POST /api/atl/credentials`** (substrate-internal namespace). Future-proofs for other ATL surfaces but introduces a namespace (`/api/atl/`) that doesn't yet have peers; awkward to anchor a new convention on one endpoint.
...
The pre-A10 stopgap's auth gate is a single env-var check in `route.ts`. Post-A10 verification is a SHA-256 hash + DB lookup + `is_active` check + `agent_id` match — substantial logic that doesn't belong in the route file. The choice is where this logic lives.

### Elected position

**In `/website/src/lib/security.ts`** alongside `validateApiKey`. A new function `validateAtlWriteToken(rawToken: string, agent_id: string): Promise<AtlWriteValidationResult>` performs the lookup and returns a discriminated result. The route's `verifyAgentIdOwnership(request, agent_id)` function's signature remains unchanged; its body is rewritten to:
...
3. Delegate to `validateAtlWriteToken(rawToken, agent_id)`.
4. Map the result to the existing `AuthGateResult` discriminated type (`{ ok: true, claims }` or `{ ok: false, reason }`).

### Why this and not the alternatives

The verification-placement question had three candidate answers:

- **(a) Inside `route.ts`** (matches the pre-A10 stopgap location). Rejected because post-A10 the auth logic is too heavy for `route.ts` — extracting the token, hashing it, looking up the row, checking active status, comparing agent_id, handling the discriminated result. `route.ts` would balloon. The route's concern is routing + HTTP-shape, not auth-mechanism.
...
export type AtlWriteValidationResult =
  | { valid: true; credential_id: string; owner_user_id: string; agent_id: string }
  | { valid: false; reason: 'no_token' | 'invalid_token' | 'revoked' | 'wrong_agent' }
...
3. Query `api_keys` filtering `key_hash = $1 AND purpose = 'atl_write' AND is_active = true`.
...
6. Otherwise return `{ valid: true, credential_id, owner_user_id, agent_id }`.
...
async function verifyAgentIdOwnership(
  ...
  const result = await validateAtlWriteToken(rawToken, agent_id);
  if (!result.valid) {
    return { ok: false, reason: 'unauthorized' };
  }
...
The function becomes `async` (was synchronous pre-A10); the call site at the route (`const auth = await verifyAgentIdOwnership(...)`) already supports this — the build session changes `const auth = verifyAgentIdOwnership(...)` to `const auth = await verifyAgentIdOwnership(...)`. The route's POST handler signature is unaffected.

### R-rule engagement

R0 (verification is part of the audit trail's authenticity guarantee — every successful write event is now traceable to a specific credential row); R4 (the validation function returns only the bound agent_id + credential identifiers; no engine internals); R17 (primary R17 engagement at the verification point); R18a (no category-language change); AC7 (ENGAGED at build session — new auth surface internals; Critical Change Protocol applies); KG1 (engaged at build — await all DB reads; the new validation function's Supabase query is awaited).
...
The revocation question had four candidate answers:

- **(a) Reuse `is_active` + add `revoked_at` timestamp.** *Adopted.* Maximum reuse of existing schema; `is_active` already does the operational work; `revoked_at` adds the audit timestamp. Lightest change.
- **(b) Separate `revoked_credentials` table.** Append-only revocation list; verifier consults. Rejected because `api_keys.is_active` already covers the operational case; a separate table duplicates the operational concern.
- **(c) No revocation; time-bounded only.** Rejected because Decision G (rotation/expiry) elects no expiry; this would leave tokens immortal-and-irrevocable. Untenable for any production auth surface.
- **(d) Both `is_active` column + audit table.** Defensive: column for fast verification, audit table records who/when/why for every issue/revoke event. *Partially adopted* — the `credential_audit` table from Decision H carries the audit events; the `revoked_at` column carries the operational timestamp. The two together approximate (d).

### Structural constraint

The schema migration (per Decision C) adds:
...
The rotation/expiry question had four candidate answers:

- **(a) No expiry; only revocation.** *Adopted.* Matches `api_keys` precedent; simplest schema; lightest build. Trade-off: a leaked credential remains valid until detected and revoked. Mitigation: the audit trail (Decision H) makes detection easier; the admin endpoint makes revocation easy.
- **(b) Long expiry (e.g., 1 year) + manual renewal.** Hardens against forgotten-credential scenarios at the cost of an annual renewal flow. Rejected as premature: the founder is the only owner pre-launch; "forgotten credentials" isn't a real risk yet. Re-evaluate post-launch.
- **(c) Short expiry + refresh-token flow.** OAuth-style. Heavy. Earns its place when tokens are high-value-per-request and frequently exposed (e.g., browser-frontend tokens). A10 tokens are server-to-server; the threat model is closer to API keys than to session tokens.
- **(d) Configurable per-credential expires_at.** Most flexible; small schema cost. Rejected for the first build to keep scope tight; trivial to add later as an `ALTER TABLE … ADD COLUMN expires_at timestamptz` migration.

### Structural constraint

No schema change for this decision (the `revoked_at` column from Decision F covers the audit timestamp; no `expires_at` is added). Verification per Decision E does not check expiry. The build session documents in `security.ts` the design intent ("A10 credentials have no expiry; renewal is revoke-and-reissue") so future changes are deliberate.

### R-rule engagement

R0 (the choice of no-expiry is itself an audit-trail consideration — the audit doesn't carry an expiry-based termination event because there is none; revocation is the only termination); R17 (the no-expiry choice is the simpler auth posture; mitigated by Decision F's revocation and Decision H's observability).
...
- **Per-credential `expires_at` column** (option (d)). Revisit condition: a use case surfaces requiring time-limited credentials (e.g., temporary delegations, CI integrations with finite project lifetimes).
- **Refresh-token flow.** Revisit condition: the threat model shifts (e.g., A10 tokens are exposed in browser contexts; tokens leak via logs more frequently than expected).
- **Automated rotation reminders.** Revisit condition: customer feedback asks for rotation prompts.
...
The audit-trail question had four candidate answers:

- **(a) Structured JSON to `console.log` only.** All three event classes emit Vercel-logs events. Rejected because Vercel logs retain only ~30 days (per the project's retention) and issuance + revocation events are forensically important enough to need durable storage. A leaked credential investigated 60 days later loses the issuance trail.
- **(b) New Supabase audit table only.** All events go to `credential_audit`. Rejected because verification fires on every write request — at production scale this would grow the table by potentially millions of rows. The asymmetric rates argue against a single-table approach.
- **(c) Both Vercel logs (verification) + Supabase audit table (issuance/revocation).** *Adopted.* Matches the event rates: Vercel logs are right for high-frequency operational signals (cheap to write; queryable via Vercel's log UI); Supabase rows are right for low-frequency forensic events (durable; SQL-queryable).
- **(d) Minimal (no app-level logging).** Rely on Supabase's native query logs + Vercel's native request logs. Rejected as too thin — neither captures the app-level context (which credential failed verification; which owner issued which credential).
...
```json
{
  ...
  "outcome": "ok" | "no_token" | "invalid_token" | "wrong_agent" | "not_enabled",
...
CREATE TABLE IF NOT EXISTS credential_audit (
  ...
  actor_user_id uuid REFERENCES auth.users(id), -- the admin or owner who triggered the event
  agent_id text NOT NULL,
  details jsonb, -- optional context: { reason?: string, label?: string, tier?: string, etc. }
...
Writes:
- Issuance handler writes one `event_type = 'issue'` row per successful mint, capturing `actor_user_id` (the admin), `agent_id` (the bound id), `details` (the label, tier, etc.).
- Revocation handler writes one `event_type = 'revoke'` row per successful revoke, capturing `actor_user_id` (the admin), `agent_id` (the bound id), `details` (the suspension reason).
...
The stopgap-retirement question had three candidate answers:

- **(a) Remove the env-flag check entirely.** Cleaner code; A10 token verification is the sole gate. Rejected because A10 is an auth surface — emergency lockdown capability has high value at near-zero cost. If the credential store has an outage (Supabase issue causing spurious accepts/rejects), if there's a misconfigured `purpose` filter, if a future code change accidentally widens access, the founder can flip one env var from a phone and the whole write surface stops. No deploy needed.
- **(b) Keep as kill-switch.** *Adopted.* Defence-in-depth at minimum cost.
- **(c) Staged-rollout mechanism (flag governs route exposure; A10 governs writes).** Both must be satisfied. Rejected as over-architected for "no current users" — collapses into (b) in practice; the dual-meaning ("exposed" vs "writes-allowed") is harder to reason about than a single kill-switch semantics.
...
The build session implements all nine decisions. Expected risk classification: **Critical** under 0d-ii (the auth-gate swap engages AC7 + R17; new admin endpoint engages AC7; schema additions engage AC7; full Critical Change Protocol applies). PR1 single-build proof: schema + library + admin endpoint + route auth-gate swap + tests all land in one session.

| File | Change |
|---|---|
| `/website/supabase-api-keys-a10-migration.sql` (NEW) | Decisions B + C + F — additive idempotent migration: `agent_id`, `owner_user_id`, `purpose`, `revoked_at` columns on `api_keys`; unique index on `(owner_user_id, agent_id, purpose) WHERE purpose = 'atl_write'`; index on `(purpose, agent_id) WHERE purpose = 'atl_write'`. |
| `/website/supabase-credential-audit-migration.sql` (NEW) | Decision H — `credential_audit` table; three indexes. |
| `/website/src/lib/security.ts` (MODIFIED) | Decisions A + B + D + E + F + H — add `AtlWriteValidationResult` type; add `validateAtlWriteToken(rawToken, agent_id)` async function (Decision E body); add `generateAtlWriteToken()` helper (Decision D); add `logAtlVerifyEvent(event)` helper (Decision H verification-path logger); add `requireAdmin(request)` helper (Decision D — admin check, OR inline in the route's handler; build-session discretion); existing `validateApiKey` and `requireAuth` unchanged. |
| `/website/src/app/api/admin/accreditation-credentials/route.ts` (NEW) | Decisions B + D + F + H — `POST` handler (mint flow); `DELETE` handler (revoke flow); auth-gated via `requireAuth` + admin check; both write `credential_audit` rows; response shapes per Decision D. |
| `/website/src/app/api/accreditation/[agent_id]/route.ts` (MODIFIED) | Decisions A + E + I — `verifyAgentIdOwnership` becomes `async`; body rewritten: kill-switch check (Decision I) → token extraction → `validateAtlWriteToken` delegation (Decision E); existing call site updated to `await` the function; header comments updated to describe post-A10 behaviour. The POST handler's outer try/catch already handles the async path. |
| `/website/src/lib/__tests__/security.test.ts` (MODIFIED or NEW) | Decisions A + E + F — tests for `validateAtlWriteToken`: token-prefix-rejection (no_token); unknown-hash (invalid_token); revoked credential (invalid_token — fails on `is_active = false` filter); wrong-agent (the credential is for agent_X but caller wrote to agent_Y); successful validation. Uses a Supabase test fixture or mock per existing pattern. |
| `/website/src/app/api/accreditation/[agent_id]/__tests__/route.test.ts` (MODIFIED) | Decisions A + E + I — the POST tests' setup needs to mock or provision a valid credential; the auth-gate tests now exercise the A10 verification path; the `unauthorized` test verifies 401 for unknown / revoked / wrong-agent tokens; the `not_enabled` test verifies 503 still fires when the kill-switch is off (Decision I). |
| `/website/src/app/api/admin/accreditation-credentials/__tests__/route.test.ts` (NEW) | Decisions B + D + F + H — mint + revoke tests: founder-only auth (401 for non-admin); successful mint returns raw token + structured response; mint writes `credential_audit` row; mint enforces unique constraint (409 on duplicate); revoke sets `is_active = false`, `revoked_at`, `suspended_reason`; revoke writes `credential_audit` row. |
...
- The bespoke election is justified for A10's substrate-internal auth surface: the existing `api_keys` infrastructure IS the load-bearing Anthropic-adjacent primitive being reused. The build session's PR15 consult records this — A10 is not a bespoke design, it's an extension of the production-tested ecosystem-API auth pattern.

---

## Cross-references

- `/operations/decision-log.md` — `D-ATL-A10-DESIGN-LOCKED-2026-05-16` (this design's adoption record).
- `/operations/handoffs/founder/2026-05-16-A10-design-pass-close.md` — this session's close (paired with the decision-log entry).
- `/operations/handoffs/founder/2026-05-16-write-path-build-close.md` — immediate predecessor session close.
- `/operations/handoffs/founder/2026-05-15-post-build-brainstorm-close.md` — sequencing source (step 8 of 8 in the post-6b arc).
- `/adopted/atl-write-path-design.md` — Decision C of the write-path design names A10 as the auth-seam filler; A10's Decision E names the swap target (`verifyAgentIdOwnership`).
- `/adopted/atl-kathekon-aligned-alternative-design.md` — structural template (seven-decision design-pass shape; this design extends to nine decisions).
- `/adopted/adr/2026-05-12-substrate-category-character-kernel.md` — J1 ADR (Character Kernel category language; preserved by this design).
...
- `/website/src/lib/security.ts` — the existing `validateApiKey` + `hashKey` + `extractRawKey` + `requireAuth` precedents that A10 reuses (Decision E + Decision D).
...
- `/website/src/app/api/accreditation/[agent_id]/route.ts` — the existing write surface; A10 swaps `verifyAgentIdOwnership`'s body (Decisions A + E + I) without changing the call site.
- `/website/src/lib/substrate/atl-accreditation-writer.ts` — the write-path library; unchanged by this design.
- `/manifest.md` — R0 (audit trail; load-bearing for Decisions B + F + H), R3 (disclaimer posture preserved; non-leaking error responses), R4 (IP boundary preserved; tokens carry no engine internals), R17 (primary engagement — Decisions A + B + E + F + H), R18a (Character Kernel credential's integrity protected; no category-language change), R18b (badge documentation update deferred under PR7), R18c (interoperability preserved at the payload layer; not at the token layer), R18e (NOT engaged at credential level), R20 (NOT engaged — no distress surface, no R20a perimeter touch), AC5 (NOT engaged — no R20a perimeter change), AC7 (ENGAGED at build session — new auth surface; Critical Change Protocol applies; full template), AC8 (translation-sandwich substrate; A10 is auth-layer, not Layer 1 contract), AC10 (`credential_audit` is upstream provenance for A12's AC10 implementation; PR7 cross-reference), KG1 (engaged at build session — every DB write/read awaited; no self-calls between the admin endpoint and the auth endpoint; no fire-and-forget on audit writes), KG7 (NOT engaged — no JSONB writes at the credential layer; `details` column is JSONB but holds free-form context, not Layer 1 payload), PR1 (single-build proof — A10's schema + library + admin endpoint + route swap + tests land in one session), PR2 (build-to-wire immediate at the build session — tests invoke the new code paths in the same session), PR4 (N/A — no LLM call), PR6 (NOT engaged — no R20a / distress-classifier surface), PR7 (deferred items named — multi-owner flows, ownership transfer, self-service mint, multiple-tokens-per-agent, token re-display, expires_at column, refresh-token flow, AC10 provenance linking, audit retention, per-IP failure rate-limiting, per-credential kill-switch, kill-switch dashboard, ecosystem-key `owner_user_id` backfill, `purpose` CHECK constraint, reactivation, suspension-reason taxonomy, VC interop at the token layer, JWT for external verifiers, validation caching, per-credential rate-limiting), PR10 (Plan → Execute → Verify — this session is the **Plan** step; A10 build session is **Execute**; founder post-deploy verification is **Verify**), PR11 (inbox scan recorded — no new files since predecessor close; F4 named in the cross-references), PR15 (Anthropic-primitive consult — bespoke election justified for A10: the existing `api_keys` + `validateApiKey` infrastructure is the production-adjacent reusable primitive; `mcp-builder` is a forward pointer for R18c interoperability post-launch).

---

*End of design document. Status: Adopted 2026-05-16 (decision); Designed (implementation). The A10 build session opens against this document + `D-ATL-A10-DESIGN-LOCKED-2026-05-16` as the spec. After the A10 build session lands, the post-6b arc closes — the substrate carries authenticated read AND write public surfaces, both auditable; every credential write traces to a specific agent_id whose identity has been verified at the route boundary.*
