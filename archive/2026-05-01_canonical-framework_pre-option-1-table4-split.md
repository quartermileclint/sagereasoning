...
**Implements:** AC-8 (single canonical mechanism framework, alt-3 architectural commitment).
...
| `REFLECTION_PROMPT` field | Canonical mechanism(s) / source | Status under alt-3 |
|---|---|---|
| `katorthoma_proximity` | mechanism 10 | **Removed at the reflect endpoint surface (AC-18).** Internal classification update only. |
| `passions_detected[]` (root_passion, sub_species, false_judgement) | mechanisms 2 + 3 + 5 | **Removed at the reflect endpoint surface (AC-18).** |
| `what_you_did_well` | Layer 3 prose translation of mechanism 9 (positive virtue engagement) | **Removed at the reflect endpoint surface (AC-18).** |
| `sage_perspective` | Layer 3 prose translation of `improvement_path` | **Removed at the reflect endpoint surface (AC-18).** |
| `evening_prompt` | Today: LLM-composed reflective question | **Removed at the reflect endpoint surface (AC-15 / AC-18).** Replaced by structured presentation of specific deferred questions from prior `OPEN_DEFERRAL` flags. No new prompt is composed. |
| `structured_observation` | Mentor knowledge persistence pipeline | Continues as internal pipeline; not surfaced to the practitioner. |
| `disclaimer` | R3 disclaimer | Carried in API response envelope; no practitioner-facing change. |

This is the most consequential mapping. Every practitioner-visible field at the reflect endpoint today is removed under alt-3. The reflect endpoint produces:

- The internal classification update (visible in the scoring record but not as a celebratory artefact).
- The closed `OPEN_DEFERRAL` flag.

That is all. No score, no perspective, no prompt, no observation. The practitioner cannot point to a completed reflection; the only evidence of completed examination is the closing of an open deferral, which the practitioner themselves must look up if they care to.

This is the surface-level expression of AC-18 (the no-shareable-artifact constraint) and AC-15 (sub-option 1b structured intake). Deliverable 14 specifies the reflect endpoint design in full.
