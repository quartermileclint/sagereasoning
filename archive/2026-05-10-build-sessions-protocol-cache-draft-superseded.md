...
**Status:** Draft. Created 2026-05-10. Pending validation in the substrate-build-staging planning session. Intended for `/adopted/` once approved.
...
2. **Read this cache** (~3 min) for build-arc-specific context: the agreed architecture, six decisions, two governing rules, migration intent, and living-state references.
...
## The six decisions (operative summaries)
...
To these is added the post-Decision-6 founder direction:

| # | Direction | Source |
|---|---|---|
| 7 | Once Layer 1, 2, 3 are finalised and adopted, every existing SageReasoning product currently using the bundled prose method swaps to the translation-sandwich method. Migration is part of this build arc. | Founder direction 2026-05-10, captured here pending close revision |
...
| This cache | `/drafts/build-sessions-protocol-cache.md` (→ `/adopted/` once approved) | Build-arc-specific context | Every build-arc session |
...
*End of cache. This document is the build-arc-specific session-opening reference. Pending validation in the planning session, then moves to `/adopted/`. Update discipline: in-session amendment when underlying records change.*
