...
**✅ RESOLVED 2026-04-24 — Option A.**

**Execution (this session):**
- `drafts/manifest-DRAFT-2026-04-18.md` → `/archive/2026-04-18_manifest_drafts-copy.md` (29,428 bytes, byte-identical to current `/manifest.md` confirmed by `diff -q` this session)
- `drafts/2026-04-20_PROJECT_STATE_draft.md` → `/archive/2026-04-20_PROJECT_STATE_drafts-copy.md` (22,031 bytes, byte-identical to current `/PROJECT_STATE.md`)
- `/drafts/` now contains only the empty `/drafts/backup/` placeholder (harmless residue from D6-A migration; already documented in INDEX)

**Forward posture:** `/drafts/` is reserved for proposed-successor content only. Snapshots of adopted state do not live in `/drafts/`. When adopted files are updated, the prior version is archived per D6-A, not mirrored into `/drafts/`.
...
**✅ RESOLVED 2026-04-24 — Option C (raise-for-review found NOT a snapshot).**

**Execution (this session):**
- `diff -q` against `/archive/2026-04-18_Project-Instructions.md` returned **differ** (not identical). Size comparison: archive copy = 28,139 bytes; drafts copy = 34,536 bytes. The drafts copy is ~6,400 bytes LARGER than what was adopted.
- Interpretation: the drafts copy represents proposed-successor content that was NOT adopted (content was cut between the 04:48 drafts save and the 05:03 archive/adoption copy).
- Archived with a naming that signals non-adoption: `drafts/project-instructions-DRAFT-2026-04-18.md` → `/archive/2026-04-18_project-instructions_unadopted-draft.md`
- If the ~6,400 bytes of cut content needs review later, it can be diffed against the archive-of-adopted copy (`/archive/2026-04-18_Project-Instructions.md`). This is queued as an open question for a future session; no action required now.

**Forward posture:** Consistent with D1 — `/drafts/` does not carry snapshots of adopted state. Proposed-but-not-adopted drafts are archived with `_unadopted-draft` suffix so the signal that they were considered but cut is preserved in the filename.
...
**✅ RESOLVED 2026-04-24 — Option E (new option adopted this session: archive BOTH as completion records).**

**Finding that reframed the decision:** A scan of both files showed that the Revised file's own audit marked the Original's 11 phases as COMPLETE, and the Revised file also marks its own 5 phases (P12–P16) as completed 3 April 2026. Neither file describes forward work. Both are historical completion records. The original D3 options (A move-original-only, B update-rule-refs, C A+B, D audit-revised) all assumed one file might still be governing — reality is that neither is.

**Execution (this session):**
- `adopted/V3_Adoption_Scope.md` → `/archive/2026-03-30_V3_Adoption_Scope_P1-P11_completed.md` (34,502 bytes)
- `adopted/V3_Adoption_Scope_Revised_April.md` → `/archive/2026-04-02_V3_Adoption_Scope_Revised_P12-P16_completed.md` (19,371 bytes)
- File-date naming chosen (each file archived under its own mtime) for faithful chronology.
- `.docx` companions (founder directive "delete docx if md equivalent exists, proceed"):
  - `adopted/V3_Adoption_Scope.docx` — md equivalent present in archive → **deleted** (20,462 bytes).
  - `adopted/V3_Adoption_Scope_Addendum_P12-P19.docx` — no md equivalent (addendum P12-P19 ≠ revised P12-P16) → **archived** to `/archive/2026-03-31_V3_Adoption_Scope_Addendum_P12-P19_completed.docx` (27,262 bytes) to preserve the only copy of the P17-P19 content.
  - Sandbox note: `rm` was initially blocked ("Operation not permitted" on the FUSE mount); deletion was enabled for the `sagereasoning` folder via the `allow_cowork_file_delete` permission tool before proceeding.
- `/adopted/` is now empty of governing files; `backup/` subfolder remains as a harmless D6-A migration placeholder.

**Cross-reference updates:**
- `/INDEX.md` `/adopted/` key-contents row updated to reflect `.md` files archived; `.docx` companions flagged as pending founder decision.
- `/website/public/component-registry.json` entry `doc-v3-adoption`: `path` updated to the new archive location; `desc` amended to describe the file's status as a completion record; `notes` extended with pointer to the original scope archive entry.
- `/operations/discrepancy-sort-2026-04-23.md` (this file) updated with resolution marker and Summary table.
- `/operations/decision-log.md` — new entry DD-2026-04-24-03.

**Cross-references NOT updated (historical record — correct as-is):**
- Older handoff notes and the prior discrepancy-sort entry itself (they document the state at the time).

**Outstanding micro-item:** Two `.docx` companions remain in `/adopted/` (`V3_Adoption_Scope.docx`, `V3_Adoption_Scope_Addendum_P12-P19.docx`). Same archive/retain decision pending founder input — logged as an open question.

**Forward posture:** `/adopted/` carries currently-governing docs only. Completion records for superseded/complete scopes live in `/archive/` with `_completed` suffix so the status is visible in the filename.
...
**✅ RESOLVED 2026-04-24 — Option D (trim to governance-only now; Option C auto-generation deferred under PR7).**

**Reasoning for D over A:** INDEX has drifted at least twice (Apr 11 → Apr 23 → Apr 24). The drift pattern is structural — INDEX tried to track project-state and code-directory status in addition to governance navigation, and those changed faster than INDEX could. A pure refresh (Option A) would have bought us 2–3 weeks before the next drift cycle. Option D changes INDEX's scope so the drift surface is dramatically reduced.

**Reasoning for deferring C:** Auto-generating INDEX from a file-tree scan (Option C) would eliminate drift permanently, but (a) the descriptions — what a folder is *for*, not just its name — still need manual curation, making it a 70%-auto/30%-manual hybrid rather than a pure structural fix; (b) the build cost is 1–2 sessions; (c) we don't yet know whether the trimmed INDEX will drift at all. Better to watch the trimmed version over the next 4–6 weeks and build C only if drift continues.

**Execution (this session):**
- `/INDEX.md` rewritten to governance-only scope. Removed: Root Files status table, detailed folder-structure listings for business/marketing/product/engineering/prototypes/brand/templates/manuals, Code Directories status table, Operational Directories status table. Retained: Inbox/Outbox, Governance Files, Governance Corpus Folders, Archive Protocol, Handoff Structure, Quick Reference.
- Added pointers out: `PROJECT_STATE.md` for current project status, `summary-tech-guide.md` + addendum for current technical state, `SageReasoning_Ecosystem_Map.html` for the full visual inventory.
- Added a "What moved out of INDEX in the D4-D trim" section for audit clarity — the next session can see what was removed without opening the archive.
- Prior version preserved at `/archive/2026-04-24_INDEX_pre-D4-trim.md`.

**Backlog item (PR7):** Option C — auto-generated INDEX via file-tree scan. Revisit condition: if the trimmed INDEX is found stale by more than one claim in any session opening during the next 4–6 weeks, build C. Tracked in `/operations/decision-log.md` as part of DD-2026-04-24-05.
...
|---|---|---|---|
| D1 | `/drafts/` byte-identical to adopted | Material | **✅ Resolved — Option A** |
