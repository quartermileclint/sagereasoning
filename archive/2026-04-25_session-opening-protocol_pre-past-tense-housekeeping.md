...
**1. Tier declaration.**
State the session's stream focus (founder / ops / tech / growth / support / mentor) and which canonical-source tier is being read. Tiers are defined in `/adopted/canonical-sources.md` §"Tier assignment". Every session reads sources 1, 2, 3. Governance sessions add 4, 5, 6. Code sessions add 8, 9. Orientation sessions add 7.
...
The canonical-sources file (`/adopted/canonical-sources.md`) is maintained separately so additions or retirements of reference files do not require editing this protocol. This protocol points at the list; the list carries the content.
...
If the adopted protocol produces unintended behaviour, revert by moving this file back to `/operations/outbox/session-opening-protocol-DRAFT.md` and logging a supersession entry in the decision log. If the hub-extract wiring (separate change) is also in place at the time of rollback, revert the hub route edit first per that change's rollback plan.
