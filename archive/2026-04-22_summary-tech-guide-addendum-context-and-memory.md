...
- **The `/private-mentor` page has two routing paths, not one.** (Absorbed from former knowledge-gaps register entry KG9 on 2026-04-25 under DD-2026-04-25-03.) The page name suggests a single backend at `/api/mentor/private/*`, but chat traffic actually goes through the generic `/api/founder/hub` endpoint scoped to the private hub. Chat messages POST to `/api/founder/hub` with `hub_id: 'private-mentor'` and write `mentor_interactions` rows with `interaction_type: 'conversation'`. Evening reflections POST to `/api/mentor/private/reflect` (a different endpoint), which writes a `reflections` row and then calls `updateProfileFromReflection` to write a `mentor_interactions` row with `interaction_type: 'evening_reflection'`. Bug fixes that target "the private mentor" often need to be applied at two different files. Chat-message bugs → `website/src/app/api/founder/hub/route.ts`. Evening-reflection bugs → `website/src/app/api/mentor/private/reflect/route.ts`, or upstream in `sage-mentor/profile-store.ts` if the bug is in `updateProfileFromReflection`. Verification probes must specify which path is being tested — chat vs evening reflection — and the verification prompt must filter accordingly (e.g., "most recent evening reflection entry" rather than just "most recent entry", since chat messages accumulate as most-recent between test reflections).
