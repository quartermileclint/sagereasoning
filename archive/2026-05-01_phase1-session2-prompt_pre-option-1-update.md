...
This session does not begin until the founder has signalled approval (or redo) of the three critical-path deliverables produced in the 2026-05-01 drafting session. Approval can be:

- **(a)** Approved as drafted → proceed to this session's full scope.
- **(b)** Approved with re-derivation redo on Deliverable 8 → this session is preceded by a transcript-review pass on `/drafts/rag-mentor-alt3/operationalised-rules.md`.
- **(c)** Send back specific deliverables for redesign → this session redrafts the named deliverable(s) before proceeding.
- **(d)** Defer → no session.

If the founder has not signalled approval at the open, the agent's first action is to ask which path applies. Do not proceed to design new deliverables on top of an unapproved critical path.
...
3. **`/operations/handoffs/founder/2026-05-01-rag-phase1-alt3-drafts-close.md`** — the session-1 close. Required context. Read in full.
4. **`/operations/handoffs/founder/2026-04-29e-private-mentor-rag-phase1-ALT3-close.md`** — the alt-3 architecture brief. Required context.
5. **`/drafts/rag-mentor-alt3/canonical-framework.md`** — Deliverable 2 (approved). The 9+1 mechanism taxonomy. All deliverables in this session reference it.
6. **`/drafts/rag-mentor-alt3/passion-taxonomy.md`** — Deliverable 3 (approved). The controlled vocabulary for passion mechanisms. Deliverables 10 and 13 in this session consume it.
7. **`/drafts/rag-mentor-alt3/operationalised-rules.md`** — Deliverable 8 (approved or transcript-revised). The rule book. Deliverable 9 in this session takes the dependency map summary in D8 to full treatment.
8. `/operations/decision-log.md` — at minimum the last five entries (`D-MENTOR-PIPELINE-SNAPSHOT-2026-04-29`, `D-PRIVATE-MENTOR-OBSERVER-CULL-2026-04-29`, `D-RAG-MENTOR-ALT3-ADOPTED-2026-04-29`, `D-RAG-MENTOR-ALT3-PHASE1-DRAFTS-2026-05-01`, plus the founder-approval entry if the critical path has been moved to `/adopted/`).
9. `/archive/2026-04-29_end-to-end-mentor-pipeline_snapshot.md` — the rollback baseline. Required context for Deliverable 14 (reflect endpoint design).
10. `/operations/knowledge-gaps.md` — scan KG1–KG7. KG3 (hub-label end-to-end contract) is directly relevant to Deliverable 14. KG1 (Vercel rules) becomes relevant as Deliverable 14 names the data-plane behaviour even though Phase 1 is design only.
11. `/website/src/app/api/mentor/private/reflect/route.ts` — required for Deliverable 14. Specifically the `REFLECTION_PROMPT` constant and the existing `evening_prompt` mechanism that AC-15 / AC-18 reframe. Read only; do not edit.
12. `/website/src/app/private-mentor/page.tsx` — required for Deliverable 14. Specifically the morning/evening ritual flow (`submitRitual` function, `MorningView`, `EveningView`) and `fetchProximityScore` that AC-19 sequencing replaces. Read only; do not edit.
13. `/website/src/app/api/founder/hub/route.ts` — read only, around the mentor sonnet call (~line 706) and the cull guard (~line 1497) for Phase-2 build-order context. Required for Deliverable 14's awareness of the existing code surface but not for design changes this session.
14. `/website/src/data/stoic-brain-compiled.ts` — required for Deliverable 4 (corpus inventory).
15. `/website/src/lib/context/stoic-brain-loader.ts` — required for Deliverable 4 and Deliverable 10 (Layer 1 translation specification).
16. `/stoic-brain/stoic-brain.json` — corpus index. Required for Deliverable 4.
...
Seven deliverables this session, recommended sequencing:
...
6. **Deliverable 14 — Reflect endpoint design** (`/drafts/rag-mentor-alt3/reflect-endpoint-design.md`). **The load-bearing deliverable for Phase-2 pass 1.** Specify in full:
   ...
   - **AC-18 — no-shareable-artifact constraint as architectural specification.** Document specifically what the reflect endpoint **does NOT produce**: no reflection score, no progress summary, no developmental visualisation, no shareable output of any kind. Document what it **DOES produce**: the internal classification update (visible in the scoring record but not as a celebratory artefact) and the closed OPEN_DEFERRAL flag (likewise visible but not celebrated).
   - Surface comparison: what changes from today's `/api/mentor/private/reflect` (which produces `katorthoma_proximity`, `sage_perspective`, `evening_prompt`, `what_you_did_well`, `mentor_observation`, all surfaced to the practitioner)? Today's output is removed at the practitioner-facing surface; the engine continues to produce the internal classification, but the practitioner sees nothing they can show, share, or use as evidence.
   ...
   This deliverable specifies the architectural implementation of AC-18 + AC-19. Both are non-negotiable per the alt-3 handoff. Any proposed feature that would produce a shareable artefact is rejected at design stage. Any proposed sequencing that builds the conversation surface before the reflect endpoint is rejected at design stage.
