...
**Status of this document:** Working analysis. Not adopted. No changes proposed; resolution decisions are the founder's. Each discrepancy presents 2–3 options with reasoning. The founder marks chosen resolutions in a follow-up pass; resolutions then feed into the next session's protocol-drafting work.
...
|---|---|---|
| D1 | `/drafts/` byte-identical to adopted | Material |
| D2 | `project-instructions-DRAFT-2026-04-18.md` status undocumented | Notable |
| D3 | Two V3 Adoption Scope versions in `/adopted/` | Material |
| D4 | `INDEX.md` materially stale | Material |
| D5 | `TECHNICAL_STATE.md` materially stale | Notable |
| D6 | Three archive conventions in active competing use | Material |
| D7 | Three handoff paths; no formal categorisation | Material |
| D8 | 0b handoff format vs actual 9+ sections | Notable |
| D9 | Knowledge-gaps register beyond 3-recurrence threshold | Minor |
| D10 | Decision log referenced inconsistently | Notable |
| D11 | Tech guide [DIVERGENCE] items unresolved | Notable |
| D12 | Multiple [TBD] items unresolved across guides | Notable |
| D13 | `route.ts` line count drift (1,504 vs 1,505) | Minor |
| D14 | Status vocabulary applied inconsistently | Notable |
| D15 | Tech guide 10-step vs P1–P7 sequence | Notable |
| D16 | R20a hub deferral not in decision log (PR7 gap) | Notable |
| D17 | Hub `session_prompt` is task brief, not protocol | Material (already decided) |
